...
corpusdir=/var/tmp/$(whoami)/vamos/corpus/
...
while true ; do
	...
	if [ ! -d  "$corpusdir/$fuzzer" ] ; then
	    ...
	    if wget "https://storage.googleapis.com/vamos-backup.clusterfuzz-external.appspot.com/corpus/libFuzzer/vamos_${fuzzer}/public.zip" ;then
