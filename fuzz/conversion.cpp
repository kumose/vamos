...
template <> struct ValidationFunctionTrait<UtfEncodings::UTF16BE> {
  static inline auto Validation = &vamos::implementation::validate_utf16be;
  static inline auto ValidationWithErrors =
      &vamos::implementation::validate_utf16be_with_errors;
  static inline std::string ValidationWithErrorsName{
      NAMEOF(&vamos::implementation::validate_utf16be_with_errors)};
  static inline std::string ValidationName{
      NAMEOF(&vamos::implementation::validate_utf16be)};
  using RawType = char16_t;
};
template <> struct ValidationFunctionTrait<UtfEncodings::UTF16LE> {
  static inline auto Validation = &vamos::implementation::validate_utf16le;
  static inline auto ValidationWithErrors =
      &vamos::implementation::validate_utf16le_with_errors;
  static inline std::string ValidationWithErrorsName{
      NAMEOF(&vamos::implementation::validate_utf16le_with_errors)};
  static inline std::string ValidationName{
      NAMEOF(&vamos::implementation::validate_utf16le)};
  using RawType = char16_t;
};
template <> struct ValidationFunctionTrait<UtfEncodings::UTF32> {
  static inline auto Validation = &vamos::implementation::validate_utf32;
  static inline auto ValidationWithErrors =
      &vamos::implementation::validate_utf32_with_errors;
  static inline std::string ValidationWithErrorsName{
      NAMEOF(&vamos::implementation::validate_utf32_with_errors)};
  static inline std::string ValidationName{
      NAMEOF(&vamos::implementation::validate_utf32)};
  using RawType = char32_t;
};
template <> struct ValidationFunctionTrait<UtfEncodings::UTF8> {
  static inline auto Validation = &vamos::implementation::validate_utf8;
  static inline auto ValidationWithErrors =
      &vamos::implementation::validate_utf8_with_errors;
  static inline std::string ValidationWithErrorsName{
      NAMEOF(&vamos::implementation::validate_utf8_with_errors)};
  static inline std::string ValidationName{
      NAMEOF(&vamos::implementation::validate_utf8)};
  using RawType = char;
};
...
struct Conversion {
  ...
  using ConversionResult =
      std::invoke_result<ConversionFunction, const vamos::implementation*,
  ...
  validation_result verify_valid_input(FromSpan src) const {
    ...
    std::vector<vamos::result> results;
    ...
    for (auto impl : implementations) {
      ...
      const bool validation1 = results.back().error == vamos::SUCCESS;
    ...
    ret.valid = std::ranges::all_of(results, [](const vamos::result& r) {
      return r.error == vamos::SUCCESS;
    });
    return ret;
  }
  ...
  validation_result verify_valid_input(FromSpan) const {
    // all latin1 input is valid. there is no vamos validation function for
    // it.
  ...
  // length of it.
  template <typename Dummy = void>
    requires std::is_invocable_v<LengthFunction, const vamos::implementation*,
                                 const FromType*, std::size_t>
  std::size_t invoke_lengthcalc(const vamos::implementation* impl,
                                FromSpan src) const {
    ...
    for (std::size_t i = 0; i < implementations.size(); ++i) {
      ...
      const auto success = [](const ConversionResult& r) -> bool {
        if constexpr (std::is_same_v<ConversionResult, std::size_t>) {
          ...
          return r.error == vamos::error_code::SUCCESS;
        }
      }(implret1);
  ...
  void dump_testcase(FromSpan typedspan, std::ostream& os) const {
    ...
    if constexpr (From != UtfEncodings::LATIN1) {
      ...
      os << "   "
            ...
            "validation2);\n";
      os << '\n';
      os << " if(validation1.error!= vamos::error_code::SUCCESS) {return;}\n";
    }

    if (std::is_invocable_v<LengthFunction, const vamos::implementation*,
                            const FromType*, std::size_t>) {
      os << "const auto outlen=implementation." << lengthcalcname << "((const "
         << nameoftype(FromType{}) << "*) data,\n data_len);\n";
    } else if (std::is_invocable_v<LengthFunction,
                                   const vamos::implementation*,
                                   std::size_t>) {
    ...
    if constexpr (std::is_same_v<ConversionResult, vamos::result>) {
      os << " ASSERT_EQUAL(r.error,vamos::error_code::SUCCESS);\n";
