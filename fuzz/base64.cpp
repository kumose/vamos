...
void decode_safe(std::span<const FromChar> base64_, const auto selected_option,
  ...
  const auto convertresult = vamos::base64_to_binary_safe(
  ...
  case vamos::error_code::OUTPUT_BUFFER_TOO_SMALL: {
  ...
  case vamos::error_code::INVALID_BASE64_CHARACTER: {
    assert(convertresult.count < base64.size());
  } break;
  case vamos::error_code::BASE64_INPUT_REMAINDER: {
    if (!(convertresult.count <= base64.size())) {
...
struct roundtripresult {
  ...
  std::size_t written{};
  vamos::result convertbackresult{};
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ...
  const auto selected_last_chunk =
      (selected_option == vamos::base64_url ||
       selected_option == vamos::base64_default_no_padding)
          ? vamos::last_chunk_handling_options::loose
          : [](auto index) {
