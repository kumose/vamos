...
inline std::span<const vamos::implementation* const>
get_supported_implementations() {
  static const auto impl = []() -> auto {
    std::vector<const vamos::implementation*> ret;
...
inline bool operator!=(const vamos::result& a, const vamos::result& b) {
  return a.count != b.count || a.error != b.error;
}
inline bool operator==(const vamos::result& a, const vamos::result& b) {
  return a.count == b.count && a.error == b.error;
}
auto operator<=>(const vamos::result& a, const vamos::result& b) {
  return std::tie(a.error, a.count) <=> std::tie(b.error, a.count);
}

inline std::ostream& operator<<(std::ostream& os, const vamos::result& a) {
