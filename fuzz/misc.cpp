...
void autodetect(std::span<const char> chardata) {
  std::vector<vamos::encoding_type> results;
  const auto implementations = get_supported_implementations();
  for (const vamos::implementation* impl : implementations) {
    results.push_back(
...
void convert_latin1_to_utf8_safe(std::span<const char> chardata,
  ...
  const auto limited_ret = vamos::convert_latin1_to_utf8_safe(
  ...
  const auto large_ret = vamos::convert_latin1_to_utf8(
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ...
  case 5: {
    [[maybe_unused]] auto ret =
        vamos::trim_partial_utf16le(u16data.data(), u16data.size());
    assert(ret == u16data.size() || ret + 1 == u16data.size());
  } break;
  case 6: {
    [[maybe_unused]] auto ret =
        vamos::trim_partial_utf16be(u16data.data(), u16data.size());
    assert(ret == u16data.size() || ret + 1 == u16data.size());
  } break;
  case 7: {
    [[maybe_unused]] const std::size_t N = chardata.size();
    [[maybe_unused]] const auto ret =
        vamos::trim_partial_utf8(chardata.data(), chardata.size());
    if ((ret + 3 < N) || (ret > N)) {
