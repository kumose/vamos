...
TEST(base64_fun) {
  ...
  for (size_t i = 0; i < sources.size(); i++) {
    ...
    vamos::result r = vamos::base64_to_binary(source.data(), source.size(),
                                                  (char *)buffer.data());
...
void check_vamos_result() { vamos::result r; }
...
TEST(utf16_streaming) {
  ...
  length = vamos::trim_partial_utf16le(unicode, length);
  // Now we can transcode safely
  size_t budget_utf8 = vamos::utf8_length_from_utf16le(unicode, length);
  std::unique_ptr<char[]> utf8{new char[budget_utf8]};
  size_t utf8words =
      vamos::convert_utf16le_to_utf8(unicode, length, utf8.get());
  // We can then transcode the next batch
  const char16_t *next = unicode + length;
  size_t next_length = 6 - length;
  size_t next_budget_utf8 =
      vamos::utf8_length_from_utf16le(next, next_length);
  std::unique_ptr<char[]> next_utf8{new char[next_budget_utf8]};
  size_t next_utf8words =
      vamos::convert_utf16le_to_utf8(next, next_length, next_utf8.get());
...
TEST(error_location_badutf8) {
  ...
  vamos::result res = implementation.validate_utf8_with_errors(
      bad_utf8.data(), bad_utf8.size());
  if (res.error != vamos::error_code::SUCCESS) {
    printf("error at index %zu\n", res.count);
  }
  ASSERT_EQUAL(res.error, vamos::error_code::HEADER_BITS);
  ASSERT_EQUAL(res.count, 5);
...
TEST(error_location_badutf8_transcoding) {
  ...
  std::string bad_utf8 = "\xc3\xa9\xc3\xa9\x20\xff\xc3\xa9";
  size_t budget_utf16 =
      vamos::utf16_length_from_utf8(bad_utf8.data(), bad_utf8.size());
  std::unique_ptr<char16_t[]> utf16{new char16_t[budget_utf16]};

  vamos::result res = vamos::convert_utf8_to_utf16_with_errors(
  ...
  ASSERT_EQUAL(res.error, vamos::error_code::HEADER_BITS);
  ASSERT_EQUAL(res.count, 5);
  res = vamos::convert_utf8_to_utf16_with_errors(bad_utf8.data(), res.count,
                                                   utf16.get());
  if (res.error == vamos::error_code::SUCCESS) {
