...
void print_tests(FILE *file) {
  ...
  for (const auto &test : vamos::test::test_procedures()) {
    fprintf(file, "- %s\n", test.name.c_str());
  }
}
...
void run(const CommandLine &cmdline) {
  ...
  for (const auto &implementation : vamos::get_available_implementations()) {
    ...
    auto filter = [&cmdline](const vamos::test::test_entry &test) -> bool {
    ...
    for (auto test : vamos::test::test_procedures()) {
      if (filter(test)) {
        test(*implementation);
      }
    }
