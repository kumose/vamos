...
TEST(issue_483) {
  ...
  ASSERT_EQUAL(validation1.error, vamos::error_code::TOO_LONG);
  ...
  ASSERT_EQUAL(r.error, vamos::error_code::TOO_LONG);
...
TEST(issue_448) {
  ...
  ASSERT_EQUAL(r.error, vamos::HEADER_BITS);
...
TEST(issue_213) {
  ...
  vamos::result r =
      vamos::convert_utf8_to_utf16be_with_errors(buf + 2, 1, buffer.get());
  ASSERT_TRUE(r.error != vamos::SUCCESS);
...
TEST_LOOP(trials, convert_pure_ASCII) {
  ...
  auto procedure = [&implementation](const char *utf8, size_t size,
    ...
    vamos::result res = implementation.convert_utf8_to_utf16be_with_errors(
