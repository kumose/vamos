...
using vamos::tests::helpers::transcode_utf16_to_utf32_test_base;
...
TEST(issue_532) {
  ...
  ASSERT_EQUAL(validation1.error, vamos::error_code::SURROGATE);
  ...
  ASSERT_EQUAL(r.error, vamos::error_code::SURROGATE);
...
TEST(allow_empty_input) {
  ...
  ASSERT_EQUAL(ret.error, vamos::error_code::SUCCESS);
}
...
TEST(convert_fails_if_there_is_surrogate_pair_is_followed_by_high_surrogate) {
  ...
  for (size_t i = 0; i < size - 2; i++) {
    auto procedure = [&implementation, &i](const char16_t *utf16, size_t size,
                                           char32_t *utf32) -> size_t {
      vamos::result res = implementation.convert_utf16le_to_utf32_with_errors(
          utf16, size, utf32);
      ASSERT_EQUAL(res.error, vamos::error_code::SURROGATE);
