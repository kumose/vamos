...
TEST(issue_convert_utf8_to_latin1_with_errors_cbf29ce4842223ed) {
  ...
  */
  ...
  ASSERT_EQUAL(r.error, vamos::error_code::TOO_SHORT);
}
...
TEST_LOOP(trials, too_large_input) {
  ...
  vamos::tests::helpers::RandomIntRanges random({{0xff, 0x10FFFF}}, seed);
  ...
  for (unsigned int i = 0; i < fix_size; i++) {
    ...
    if (byte_number != 1) {
      ...
      auto procedure = [&implementation, &i](const char *utf8, size_t size,
                                             char *latin1) -> size_t {
        vamos::result res = implementation.convert_utf8_to_latin1_with_errors(
            utf8, size, latin1);
...
TEST_LOOP(trials, header_bits_error) {
  vamos::tests::helpers::RandomIntRanges random({{0x0000, 0xff}}, seed);
  transcode_utf8_to_latin1_test_base test(random, fix_size);
