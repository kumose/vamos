...
static void print_input(const std::string &s,
                        const vamos::implementation *const e) {
...
int validate_tests(const char *databytes, size_t size_in_bytes) {
  ...
  vamos::result reference_result{};
  const vamos::implementation *reference_impl{};
...
bool fuzz_this(const char *data, size_t size) {
  ...
  for (auto &e : vamos::get_available_implementations()) {
    ...
    if (validutf8 != (rutf8.error == vamos::SUCCESS)) { // they should agree
    ...
    auto rutf16le = e->validate_utf16le_with_errors((char16_t *)source.c_str(),
                                                    source.size() / 2);
    if (validutf16le !=
        (rutf16le.error == vamos::SUCCESS)) { // they should agree
      print_input(source, e);
      return false;
    }
    if (validutf16le) {
    ...
    auto rutf16be = e->validate_utf16be_with_errors((char16_t *)source.c_str(),
                                                    source.size() / 2);
    if (validutf16be !=
        (rutf16be.error == vamos::SUCCESS)) { // they should agree
      print_input(source, e);
      return false;
    }
    if (validutf16be) {
    ...
    /// expect it to fail.
    {
      ...
      if (r.error == vamos::error_code::SUCCESS) {
        ...
        if (r2.error != vamos::error_code::SUCCESS) {
          print_input(source, e);
          return false;
        }
    ...
    // Same as above, but we use the safe decoder version.
    {
      ...
      if (r.error == vamos::error_code::SUCCESS) {
        ...
        vamos::result r2 = vamos::base64_to_binary_safe(
    ...
    /// it, it should always succeed.
    {
      ...
      vamos::result r = e->base64_to_binary(base64buffer.data(),
