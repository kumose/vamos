...
template <typename Pointer> bool has_bom(Pointer *data, size_t size) {
  return (vamos::BOM::check_bom(data, size) !=
          vamos::encoding_type::unspecified);
}
...
TEST_LOOP(trials, pure_utf16_ASCII) {
  vamos::tests::helpers::RandomInt random(0, 127, seed);
...
TEST_LOOP(trials, pure_utf32_ASCII) {
  vamos::tests::helpers::RandomInt random(0, 0x7f, seed);

  for (size_t size : input_size) {
    const auto generated = generate_u32(random, size / 4);
    auto expected = vamos::encoding_type::UTF8 |
                    vamos::encoding_type::UTF16_LE |
...
TEST_LOOP(trials, no_utf8_bytes_no_surrogates) {
  ...
  for (size_t size : input_size) {
    const auto generated = generate_u32(random, size / 4);
    auto expected =
        vamos::encoding_type::UTF16_LE | vamos::encoding_type::UTF32_LE;
    auto actual = implementation.detect_encodings(
        reinterpret_cast<const char *>(generated.data()), size);
    ASSERT_EQUAL((actual & expected),
...
TEST_LOOP(trials, two_utf8_bytes) {
  vamos::tests::helpers::random_utf8 random(seed, 0, 1, 0, 0);
  ...
  for (size_t size : input_size) {
    ...
    auto actual = implementation.detect_encodings(
        reinterpret_cast<const char *>(generated.data()), size);
    if (actual != expected) {
      if ((actual & vamos::encoding_type::UTF8) == 0) {
        puts("failed to detect valid UTF-8.");
      }
      if ((actual & vamos::encoding_type::UTF16_LE) == 0) {
...
TEST_LOOP(trials, utf_16_surrogates) {
  ...
  for (size_t size : input_size) {
    ...
    auto expected = vamos::encoding_type::UTF16_LE;
    auto actual = implementation.detect_encodings(
        reinterpret_cast<const char *>(generated.data()), size);
...
TEST_LOOP(trials, utf32_surrogates) {
  vamos::tests::helpers::RandomInt random_prefix(0x10000, 0x10ffff, seed);
  vamos::tests::helpers::RandomInt random_suffix(0xd800, 0xdfff, seed);
  ...
  for (size_t size : input_size) {
    ...
    auto expected = vamos::encoding_type::UTF32_LE;
    auto actual = implementation.detect_encodings(
        reinterpret_cast<const char *>(generated.data()), size);
...
TEST_LOOP(trials, edge_surrogate) {
  vamos::tests::helpers::RandomInt random(0x10000, 0x10ffff, seed);
  ...
  while (i + 32 < (size / 2) - 1) {
    char16_t W1;
    char16_t W2;
    ASSERT_EQUAL(vamos::tests::reference::utf16::encode(random(), W1, W2), 2);
    generated[i] = W1;
    generated[i + 1] = W2;
    i += 32;
  }
...
TEST_LOOP(trials, tail_utf8) {
  vamos::tests::helpers::random_utf8 random(seed, 0, 0, 1, 0);
