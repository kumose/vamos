...
TEST(test_empty) {
  ...
  for (std::size_t inputstoragesize : {0, 1}) {
    ...
    auto is_valid = [](const vamos::result &r) {
      return r.count == 0 && r.error == vamos::error_code::SUCCESS;
    };
