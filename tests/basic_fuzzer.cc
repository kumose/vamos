...
public:
  ...
  size_t next(std::vector<char> &output) {
    ...
    case ONE_VALID: {
      vamos::tests::reference::utf8::encode(generate(0x0, 0x7f), consume);
      count = 1;
      break;
    }
    case ONE_TOO_LONG: {
    ...
    case TWO_VALID: {
      vamos::tests::reference::utf8::encode(generate(0x80, 0x7ff), consume);
      count = 2;
      break;
    }
    case TWO_HEADER: {
    ...
    case THREE_SURROGATE: {
      vamos::tests::reference::utf8::encode(generate(0xd800, 0xdfff),
                                              consume);
      count = 3;
      break;
    }
    case FOUR_VALID: {
      vamos::tests::reference::utf8::encode(generate(0x10000, 0x10ffff),
                                              consume);
      count = 4;
      break;
    }
    ...
    case FOUR_TOO_LARGE: {
      vamos::tests::reference::utf8::encode(generate(0x110000, 0x1fffff),
                                              consume);
      count = 4;
      break;
    }
    ...
    while (counter < 100000) {
      for (size_t size : input_size) {
        ...
        if (is_ok_utf8.second) {
          ...
          utf8_to_utf16.first = (r.error == vamos::SUCCESS);
          ASSERT_TRUE(expected_length > 0 &&
                      (expected_length * sizeof(char16_t) == output.size()) &&
                      expected_length == utf8_to_utf16.second);
          ...
          utf8_to_utf32.first = (r.error == vamos::SUCCESS);
          ASSERT_TRUE(expected_length > 0 &&
                      (expected_length * sizeof(char32_t) == output.size()) &&
                      expected_length == utf8_to_utf32.second);
        }
        if (is_ok_utf16.second) {
          ...
          utf16_to_utf8.first = (r.error == vamos::SUCCESS);
          ASSERT_TRUE(expected_length > 0 && expected_length == output.size() &&
                      expected_length == utf16_to_utf8.second);
        ...
        if (is_ok_utf32.second) {
          ...
          auto r = implementation.convert_utf32_to_utf8_with_errors(
              reinterpret_cast<char32_t *>(input.data()),
              input.size() / sizeof(char32_t), output.data());
          utf32_to_utf8.first = (r.error == vamos::SUCCESS);
          ...
          r = implementation.convert_utf32_to_utf16le_with_errors(
              ...
              input.size() / sizeof(char32_t),
              reinterpret_cast<char16_t *>(output.data()));
          utf32_to_utf16.first = (r.error == vamos::SUCCESS);
...
int main(int argc, char *argv[]) {
  ...
  return vamos::test::main((argc == 2) ? 1 : argc, argv);
}
