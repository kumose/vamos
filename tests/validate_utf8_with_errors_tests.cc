...
TEST(validate_utf8_with_errors_cbf29ce4842223f0) {
  ...
  */
  ...
  ASSERT_EQUAL(validation1.error, vamos::error_code::HEADER_BITS);
}
...
TEST(node48995) {
  ...
  size_t length = 1;
  vamos::result res = implementation.validate_utf8_with_errors(bad, length);
...
TEST(copyright) {
  ...
  size_t length = 2;
  vamos::result res = implementation.validate_utf8_with_errors(good, length);
  ASSERT_EQUAL(res.error, vamos::error_code::SUCCESS);
}

TEST_LOOP(num_trials, no_error) {
  vamos::tests::helpers::random_utf8 generator{seed, 1, 1, 1, 1};
...
TEST_LOOP(num_trials, too_short_error) {
  ...
  for (int i = 0; i < 512; i++) {
    if ((utf8[i] & 0b11000000) ==
      ...
      vamos::result res = implementation.validate_utf8_with_errors(
          reinterpret_cast<const char *>(utf8.data()), utf8.size());
      ASSERT_EQUAL(res.error, vamos::error_code::TOO_SHORT);
...
TEST_LOOP(num_trials, too_long_error) {
  ...
  for (unsigned int i = 1; i < 512; i++) {
    if (((utf8[i] & 0b11000000) !=
      ...
      vamos::result res = implementation.validate_utf8_with_errors(
          reinterpret_cast<const char *>(utf8.data()), utf8.size());
      ASSERT_EQUAL(res.error, vamos::error_code::TOO_LONG);
      ASSERT_EQUAL(res.count, i);
      utf8[i] = old;
    }
  }
...
TEST_LOOP(num_trials, overlong_error) {
  ...
  for (unsigned int i = 1; i < 512; i++) {
    if (utf8[i] >= 0b11000000) { // Only non-ASCII leading bytes can be overlong
      ...
      vamos::result res = implementation.validate_utf8_with_errors(
          reinterpret_cast<const char *>(utf8.data()), utf8.size());
      ASSERT_EQUAL(res.error, vamos::error_code::OVERLONG);
      ASSERT_EQUAL(res.count, i);
      utf8[i] = old;
      utf8[i + 1] = second_old;
    }
  ...
  for (unsigned int i = 1; i < 512; i++) {
    if ((utf8[i] & 0b11110000) ==
      ...
      for (int s = 0x8; s < 0xf;
        ...
        vamos::result res = implementation.validate_utf8_with_errors(
            reinterpret_cast<const char *>(utf8.data()), utf8.size());
        ASSERT_EQUAL(res.error, vamos::error_code::SURROGATE);
        ASSERT_EQUAL(res.count, i);
      }
      utf8[i] = old;
      utf8[i + 1] = second_old;
