...
#define VAMOS_BASE64URL_TESTS 1
...
TEST(roundtrip_base64_with_spaces) {
  for (size_t len = 0; len < 2048; len++) {
    ...
    for (size_t trial = 0; trial < 10; trial++) {
      ...
      std::vector<char> back(vamos::maximal_binary_length_from_base64(
          buffer.data(), buffer.size()));
      for (auto option :
           {vamos::last_chunk_handling_options::strict,
            vamos::last_chunk_handling_options::loose,
            vamos::last_chunk_handling_options::stop_before_partial}) {
        vamos::result r = implementation.base64_to_binary(
            buffer.data(), buffer.size(), back.data(), vamos::base64_default,
            option);
        ASSERT_EQUAL(r.error, vamos::error_code::SUCCESS);
      ...
      for (auto option :
        ...
        auto r = vamos::base64_to_binary_safe(
            buffer.data(), buffer.size(), back.data(), back_length,
            vamos::base64_default, option);
    ...
    for (size_t trial = 0; trial < 10; trial++) {
      ...
      for (auto option :
        ...
        vamos::result r = implementation.base64_to_binary(
            ...
            vamos::base64_default_accept_garbage, option);
        ASSERT_EQUAL(r.error, vamos::error_code::SUCCESS);
        ASSERT_EQUAL(r.count, len);
        ASSERT_TRUE(
            std::equal(back.begin(), back.begin() + len, source.begin()));
...
TEST(roundtrip_base64_url_with_garbage) {
  for (size_t len = 0; len < 2048; len++) {
    ...
    for (size_t trial = 0; trial < 10; trial++) {
      ...
      size_t size = implementation.binary_to_base64(
          source.data(), source.size(), buffer.data(), vamos::base64_url);
      buffer.resize(size);
      for (size_t i = 0; i < 5; i++) {
      ...
      for (auto option :
        ...
        vamos::result r = implementation.base64_to_binary(
            buffer.data(), buffer.size(), back.data(),
            vamos::base64_url_accept_garbage, option);
        ASSERT_EQUAL(r.error, vamos::error_code::SUCCESS);
        ASSERT_EQUAL(r.count, len);
        ASSERT_TRUE(
            std::equal(back.begin(), back.begin() + len, source.begin()));
...
TEST(base64_decode_just_one_padding_partial_safe) {
  std::vector<std::tuple<std::string, vamos::result, size_t>> test_cases = {
      {"uuuu             =", {vamos::error_code::SUCCESS, 4}, 3}};
  ...
  for (auto &p : test_cases) {
    ...
    for (auto option : {vamos::base64_options::base64_default,
                        vamos::base64_options::base64_url}) {
      for (auto chunk_option :
           {vamos::last_chunk_handling_options::stop_before_partial}) {
        for (size_t output = 3; output < buffer.size(); output++) {
          size_t written = output;
          auto result = vamos::base64_to_binary_safe(
...
TEST(base64_decode_just_one_padding_partial_generous) {
  std::vector<std::pair<std::string, vamos::result>> test_cases = {
      {"uuuu             =", {vamos::error_code::SUCCESS, 3}}};
...
TEST(base64_decode_just_one_padding_loose) {
  std::vector<std::pair<std::string, vamos::result>> test_cases = {
      {"uuuu             =",
       {vamos::error_code::INVALID_BASE64_CHARACTER, 17}}};
  ...
  for (auto &p : test_cases) {
    ...
    for (auto option : {vamos::base64_options::base64_default,
                        vamos::base64_options::base64_url}) {
      for (auto chunk_option : {vamos::last_chunk_handling_options::loose}) {
        auto result = implementation.base64_to_binary(
            input.data(), input.size(), buffer.data(), option, chunk_option);
        ASSERT_EQUAL(result.error, expected_result.error);
        ASSERT_EQUAL(result.count, expected_result.count);
...
TEST(base64_decode_just_one_padding_strict) {
  std::vector<std::pair<std::string, vamos::result>> test_cases = {
      {"uuuu             =", {vamos::error_code::BASE64_INPUT_REMAINDER, 3}}};
  std::vector<char> buffer(3);
  for (auto &p : test_cases) {
    auto input = p.first;
    auto expected_result = p.second;
    for (auto option : {vamos::base64_options::base64_default,
                        vamos::base64_options::base64_url}) {
      for (auto chunk_option : {vamos::last_chunk_handling_options::strict}) {
        auto result = implementation.base64_to_binary(
            input.data(), input.size(), buffer.data(), option, chunk_option);
        ASSERT_EQUAL(result.error, expected_result.error);
        ASSERT_EQUAL(result.count, expected_result.count);
...
TEST(base64_decode_partial_cases) {
  std::vector<std::pair<std::string, vamos::result>> test_cases = {
      {"ZXhhZg", {vamos::error_code::SUCCESS, 4}},
      {"ZXhhZg                                                                ",
       {vamos::error_code::SUCCESS, 4}},
      {"                                                                ZXhhZg",
       {vamos::error_code::SUCCESS, 68}},
  };
  std::vector<char> buffer(3);
  for (auto &p : test_cases) {
    auto input = p.first;
...
TEST(base64_decode_strict_cases) {
  std::vector<std::pair<std::string, uint64_t>> test_cases = {
      {"ZXhhZg==", vamos::error_code::SUCCESS},
      {"YWE=", vamos::error_code::SUCCESS},
      {"YWF=", vamos::error_code::BASE64_EXTRA_BITS},
      {"ZXhhZh==", vamos::error_code::BASE64_EXTRA_BITS},
      {"ZXhhZg", vamos::error_code::BASE64_INPUT_REMAINDER},
      {"ZXhhZh", vamos::error_code::BASE64_INPUT_REMAINDER},
      {"Z   X  h  h   Z h =   =", vamos::error_code::BASE64_EXTRA_BITS},
      {"ZX  h  hZg", vamos::error_code::BASE64_INPUT_REMAINDER},
      {"ZXh  hZ  h", vamos::error_code::BASE64_INPUT_REMAINDER},
  };
  std::vector<char> buffer(1024);
  for (auto &p : test_cases) {
    auto input = p.first;
    auto expected_error = p.second;
    vamos::result result = implementation.base64_to_binary(
        input.data(), input.size(), buffer.data(), vamos::base64_default,
        vamos::last_chunk_handling_options::strict);
    ASSERT_EQUAL(result.error, expected_error);
    size_t written = buffer.size();
    result = vamos::base64_to_binary_safe(
        input.data(), input.size(), buffer.data(), written,
        vamos::base64_default, vamos::last_chunk_handling_options::strict);
    ASSERT_EQUAL(result.error, expected_error);
  }
}
...
TEST(base64_decode_strict_cases_length) {
  std::vector<std::pair<std::string, vamos::result>> test_cases = {
      {"ddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd"
       ...
       {vamos::error_code::BASE64_EXTRA_BITS, 131}},
  };
  std::vector<char> buffer(1024);
  for (auto &p : test_cases) {
    auto input = p.first;
...
TEST(issue_single_bad16) {
  ...
  const auto r = implementation.base64_to_binary(
      ...
      vamos::base64_url_with_padding,
      vamos::last_chunk_handling_options::strict);
  ASSERT_EQUAL(r.error, vamos::error_code::INVALID_BASE64_CHARACTER);
...
TEST(issue_615) {
  ...
  const auto r1 =
      implementation.base64_to_binary(data.data(), data.size(), output.data(),
                                      vamos::base64_default, vamos::strict);
  ASSERT_EQUAL(r1.error, vamos::error_code::BASE64_INPUT_REMAINDER);
  ASSERT_EQUAL(r1.count, 0);
  const auto r2 = implementation.base64_to_binary(
      data.data() + 1, data.size() - 1, output.data(), vamos::base64_default,
      vamos::strict);
  ASSERT_EQUAL(r2.error, vamos::error_code::BASE64_INPUT_REMAINDER);
  ASSERT_EQUAL(r2.count, 0);
  const auto r3 = implementation.base64_to_binary(
      data.data(), data.size(), output.data(), vamos::base64_default,
      vamos::stop_before_partial);
  ASSERT_EQUAL(r3.error, vamos::error_code::SUCCESS);
  ASSERT_EQUAL(r3.count, 0);
  const auto r4 = implementation.base64_to_binary(
      data.data() + 1, data.size() - 1, output.data(), vamos::base64_default,
      vamos::stop_before_partial);
  ASSERT_EQUAL(r4.error, vamos::error_code::SUCCESS);
...
TEST(issue_kkk) {
  ...
  const auto r = implementation.base64_to_binary(
      ...
      vamos::base64_url_with_padding,
      vamos::last_chunk_handling_options::strict);
  ASSERT_EQUAL(r.error, vamos::error_code::BASE64_INPUT_REMAINDER);
...
TEST(base64_decode_complete_input) {
  ...
  for (auto option :
    ...
    auto result = implementation.base64_to_binary(
        ...
        output_buffer.data(), vamos::base64_default, option);

    ASSERT_EQUAL(result.error, vamos::error_code::SUCCESS);
  ...
  for (auto option :
    ...
    for (const std::string &input_data : test_cases) {
      ...
      auto result = implementation.base64_to_binary(
          ...
          reinterpret_cast<char *>(output_buffer.data()),
          vamos::base64_default, option);
      if (option == vamos::last_chunk_handling_options::strict) {
        ASSERT_EQUAL(result.error, vamos::error_code::BASE64_INPUT_REMAINDER);
      } else {
        ASSERT_EQUAL(result.error, vamos::error_code::SUCCESS);
        ...
        for (auto option :
          ...
          if (option == vamos::last_chunk_handling_options::strict) {
            ASSERT_EQUAL(result.error,
                         vamos::error_code::BASE64_INPUT_REMAINDER);
          } else {
            ASSERT_EQUAL(result.error, vamos::error_code::SUCCESS);
      ...
      for (size_t removed = 1; !buffer.empty() && removed <= 2; removed++) {
        ...
        for (auto option :
             {vamos::last_chunk_handling_options::strict,
              vamos::last_chunk_handling_options::stop_before_partial}) {
          std::vector<uint8_t> output_buffer(
              vamos::maximal_binary_length_from_base64(buffer.data(),
                                                         buffer.size()));
          size_t written = output_buffer.size();
          auto result = vamos::base64_to_binary_safe(
              buffer.data(), buffer.size(),
...
TEST(base64_decode_strict_mode) {
  ...
  for (const std::pair<std::string, std::string> &t : test_cases) {
    ...
    auto result = implementation.base64_to_binary(
        input_data.data(), input_data.size(),
        reinterpret_cast<char *>(output_buffer.data()), vamos::base64_default,
        vamos::last_chunk_handling_options::strict);
...
TEST(issue_520_url) {
  ...
  const auto r = implementation.base64_to_binary(
      (const char *)data.data(), data.size(), out.data(), vamos::base64_url);
  ASSERT_EQUAL(r.error, vamos::error_code::BASE64_INPUT_REMAINDER);
  ASSERT_EQUAL(r.count, 0);
}
...
TEST(issue_509) {
  ...
  const auto r = implementation.base64_to_binary(
      data.data(), data.size(), out.data(), vamos::base64_default);
  ASSERT_EQUAL(r.error, vamos::error_code::INVALID_BASE64_CHARACTER);
...
TEST(decode_non_ascii_utf16) {
  ...
  std::vector<vamos::error_code> codes = {
      vamos::error_code::INVALID_BASE64_CHARACTER};
  ...
  for (size_t i = 0; i < cases.size(); i++) {
    ...
    r = vamos::base64_to_binary_safe(cases[i].data(), cases[i].size(),
                                       buffer.data(), len);
    ASSERT_EQUAL(r.error, codes[i]);
    ASSERT_EQUAL(r.count, counts[i]);
  }
...
TEST(decode_base64_cases) {
  ...
  std::vector<vamos::error_code> codes = {vamos::error_code::SUCCESS};
...
TEST(complete_decode_base64url_cases) {
  ...
  for (std::pair<std::string, std::string> p : cases) {
    std::vector<char> buffer(implementation.maximal_binary_length_from_base64(
        p.second.data(), p.second.size()));
    vamos::result r = implementation.base64_to_binary(
        p.second.data(), p.second.size(), buffer.data(), vamos::base64_url);
    ASSERT_EQUAL(r.error, vamos::error_code::SUCCESS);
    ASSERT_EQUAL(r.count, p.first.size());
    for (size_t i = 0; i < r.count; i++) {
      ASSERT_EQUAL(buffer[i], p.first[i]);
...
TEST(encode_base64_cases_no_padding) {
  ...
  for (std::pair<std::string, std::string> p : cases) {
    std::vector<char> buffer(implementation.base64_length_from_binary(
        p.first.size(), vamos::base64_default_no_padding));
    ASSERT_EQUAL(buffer.size(), p.second.size());
    size_t s = implementation.binary_to_base64(
        p.first.data(), p.first.size(), buffer.data(),
        vamos::base64_default_no_padding);
    ASSERT_EQUAL(s, p.second.size());
    ASSERT_EQUAL(std::string(buffer.data(), buffer.size()), p.second);
  }
}

#if VAMOS_BASE64URL_TESTS
...
TEST(encode_base64url_cases) {
  ...
  for (std::pair<std::string, std::string> p : cases) {
    std::vector<char> buffer(implementation.base64_length_from_binary(
        p.first.size(), vamos::base64_url));
    ASSERT_EQUAL(buffer.size(), p.second.size());
    size_t s = implementation.binary_to_base64(
        p.first.data(), p.first.size(), buffer.data(), vamos::base64_url);
    ASSERT_EQUAL(s, p.second.size());
...
TEST(encode_base64url_with_padding_cases) {
  ...
  for (std::pair<std::string, std::string> p : cases) {
    std::vector<char> buffer(implementation.base64_length_from_binary(
        p.first.size(), vamos::base64_url_with_padding));
    ASSERT_EQUAL(buffer.size(), p.second.size());
    size_t s = implementation.binary_to_base64(
        p.first.data(), p.first.size(), buffer.data(),
        vamos::base64_url_with_padding);
...
TEST(roundtrip_base64url) {
  for (size_t len = 0; len < 2048; len++) {
    ...
    std::vector<char> buffer;
    buffer.resize(
        implementation.base64_length_from_binary(len, vamos::base64_url));
    std::vector<char> back(len);
    std::mt19937 gen((std::mt19937::result_type)(seed));
    std::uniform_int_distribution<int> byte_generator{0, 255};
    for (size_t trial = 0; trial < 10; trial++) {
      ...
      ASSERT_EQUAL(size, implementation.base64_length_from_binary(
                             len, vamos::base64_url));
      vamos::result r = implementation.base64_to_binary(
          buffer.data(), size, back.data(), vamos::base64_url);
      ASSERT_EQUAL(r.error, vamos::error_code::SUCCESS);
      ASSERT_EQUAL(r.count, len);
      if (back != source) {
        printf("=====input size %zu\n", len);
      ...
      for (auto option :
        ...
        if ((size % 4) == 0) {
          ...
          if (option == vamos::last_chunk_handling_options::strict) {
            ASSERT_EQUAL(r.error, vamos::error_code::BASE64_INPUT_REMAINDER);
          } else if (option == vamos::last_chunk_handling_options::loose) {
            ASSERT_EQUAL(r.error, vamos::error_code::SUCCESS);
            ASSERT_EQUAL(r.count, len);
            ASSERT_TRUE(back == source);
          } else if (option == vamos::last_chunk_handling_options::
                                   stop_before_partial) {
            ASSERT_EQUAL(r.error, vamos::error_code::SUCCESS);
  ...
  for (size_t len = 0; len < 2048; len++) {
    ...
    for (size_t trial = 0; trial < 10; trial++) {
      ...
      ASSERT_EQUAL(size, implementation.base64_length_from_binary(
                             len, vamos::base64_url));
      vamos::result r = implementation.base64_to_binary(
          buffer16.data(), size, back.data(), vamos::base64_url);
      ASSERT_EQUAL(r.error, vamos::error_code::SUCCESS);
      ASSERT_EQUAL(r.count, len);
      if (back != source) {
        printf("=====input size %zu\n", len);
    ...
    for (size_t trial = 0; trial < 10; trial++) {
      ...
      if (padding == 1) {
        ...
        // adding padding should break
        {
          ...
          vamos::result r =
              vamos::base64_to_binary(copy.data(), copy.size(), back.data());
          ASSERT_EQUAL(r.error, vamos::error_code::INVALID_BASE64_CHARACTER);
        }
  ...
  for (size_t len = 0; len < 2048; len++) {
    ...
    for (size_t trial = 0; trial < 10; trial++) {
      ...
      std::vector<char> back(vamos::maximal_binary_length_from_base64(
          buffer.data(), buffer.size()));
      vamos::result r =
          vamos::base64_to_binary(buffer.data(), buffer.size(), back.data());
      ASSERT_EQUAL(r.error, vamos::error_code::INVALID_BASE64_CHARACTER);
      ...
      for (auto option :
           {vamos::last_chunk_handling_options::strict,
            ...
            vamos::last_chunk_handling_options::stop_before_partial}) {
        size_t back_length = back.size();
        r = vamos::base64_to_binary_safe(buffer.data(), buffer.size(),
...
TEST(doomed_truncated_base64_roundtrip_16) {
  for (size_t len = 1; len < 2048; len++) {
    ...
    for (size_t trial = 0; trial < 10; trial++) {
      ...
      ASSERT_EQUAL(r.count, (size - 4) / 4 * 3);
      size_t back_length = back.size();
      r = vamos::base64_to_binary_safe(buffer16.data(), buffer16.size(),
                                         back.data(), back_length);
      ASSERT_EQUAL(r.error, vamos::error_code::BASE64_INPUT_REMAINDER);
...
TEST(roundtrip_base64_16_with_garbage) {
  for (size_t len = 0; len < 2048; len++) {
    ...
    for (size_t trial = 0; trial < 10; trial++) {
      ...
      vamos::result r = implementation.base64_to_binary(
          buffer16.data(), buffer16.size(), back.data(),
          vamos::base64_default_accept_garbage);
      ASSERT_EQUAL(r.error, vamos::error_code::SUCCESS);
      ASSERT_EQUAL(r.count, len);
      if (back != source) {
        printf("=====input size %zu\n", len);
...
TEST(roundtrip_base64_url_16_with_garbage) {
  for (size_t len = 0; len < 2048; len++) {
    ...
    for (size_t trial = 0; trial < 10; trial++) {
      ...
      vamos::result r = implementation.base64_to_binary(
          buffer16.data(), buffer16.size(), back.data(),
          vamos::base64_url_accept_garbage);
      ASSERT_EQUAL(r.error, vamos::error_code::SUCCESS);
      ASSERT_EQUAL(r.count, len);
      if (back != source) {
        printf("=====input size %zu\n", len);
...
TEST(streaming_base64_roundtrip) {
  ...
  for (size_t window = 16; window <= 2048; window += 7) {
    ...
    for (size_t pos = 0; pos < buffer.size(); pos += window) {
      ...
      ASSERT_TRUE(r.error != vamos::error_code::INVALID_BASE64_CHARACTER);
      if (count + pos == buffer.size()) {
        ...
        if (r.error == vamos::error_code::BASE64_INPUT_REMAINDER) {
...
TEST(readme_test) {
  ...
  for (size_t pos = 0; pos < base64.size(); pos += window) {
    ...
    vamos::result r = vamos::base64_to_binary(base64.data() + pos, count,
                                                  back.data() + outpos);
    if (r.error == vamos::error_code::INVALID_BASE64_CHARACTER) {
    ...
    if (count + pos == base64.size() &&
        r.error == vamos::error_code::BASE64_INPUT_REMAINDER) {
...
int main(int argc, char *argv[]) {
  ...
  return vamos::test::main(argc, argv);
}

#if !defined(VAMOS_NO_THREADS) && VAMOS_ATOMIC_REF
TEST(atomic_roundtrip_base64) {
  for (size_t len = 0; len < 2048; len++) {
    ...
    for (size_t trial = 0; trial < 10; trial++) {
      ...
      size_t size = vamos::atomic_binary_to_base64(
          source.data(), source.size(), buffer.data());
      ASSERT_EQUAL(size, implementation.base64_length_from_binary(len));
      vamos::result r =
          implementation.base64_to_binary(buffer.data(), size, back.data());
...
TEST(atomic_span_roundtrip_base64) {
  for (size_t len = 0; len < 2048; len++) {
    ...
    for (size_t trial = 0; trial < 10; trial++) {
      ...
      size_t size = vamos::atomic_binary_to_base64(source, buffer);
      ASSERT_EQUAL(size, implementation.base64_length_from_binary(len));
      vamos::result r =
          implementation.base64_to_binary(buffer.data(), size, back.data());
      ASSERT_EQUAL(r.error, vamos::error_code::SUCCESS);
...
#endif // !defined(VAMOS_NO_THREADS) && VAMOS_ATOMIC_REF
