...
TEST_LOOP(trials, convert_random_inputs) {
  vamos::tests::helpers::RandomInt r(0x00, 0xffff, seed);
  ...
  for (size_t size : input_size) {
    ...
    if (vamos::tests::reference::validate_utf16_to_latin1(utf16.data(),
                                                            size)) {
...
TEST(convert_fails_if_input_too_large) {
  uint32_t seed{12};
  vamos::tests::helpers::RandomInt generator(0x00ff, 0xffff, seed);
