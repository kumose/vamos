...
TEST_LOOP(
  ...
  vamos::tests::helpers::random_utf16 generator{seed, 1, 0};
  const auto utf16{generator.generate(512, seed)};

  vamos::result res = implementation.validate_utf16le_with_errors(
...
TEST_LOOP(
  ...
  vamos::tests::helpers::random_utf16 generator{seed, 0, 1};
...
TEST(provoke_integer_wraparound_in_icelake) {
  ...
  auto r = vamos::validate_utf16le_with_errors(
...
TEST(validate_utf16le_with_errors_returns_success_for_valid_input_mixed) {
  uint32_t seed{1234};
  vamos::tests::helpers::random_utf16 generator{seed, 1, 1};
  const auto utf16{generator.generate(512)};
...
TEST(validate_utf16le_with_errors_extensive_tests) {
  ...
  while (file) {
    ...
    // format: [TF][VLH]{16}
    vamos::error_code valid = vamos::error_code::SURROGATE;
    switch (line[0]) {
    case 'T':
      valid = vamos::error_code::SUCCESS;
      break;
    case 'F':
      valid = vamos::error_code::SURROGATE;
      break;
    default:
