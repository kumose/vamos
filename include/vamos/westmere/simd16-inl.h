...
struct base16 : base<simd16<T>> {
  ...
  vamos_really_inline base16() : base<simd16<T>>() {}
  vamos_really_inline base16(const __m128i _value)
      : base<simd16<T>>(_value) {}
  template <typename Pointer>
  vamos_really_inline base16(const Pointer *ptr)
      : base16(_mm_loadu_si128(reinterpret_cast<const __m128i *>(ptr))) {}

  friend vamos_really_inline Mask operator==(const simd16<T> lhs,
                                               const simd16<T> rhs) {
  ...
  template <int N = 1>
  vamos_really_inline simd16<T> prev(const simd16<T> prev_chunk) const {
...
template <> struct simd16<bool> : base16<bool> {
  static vamos_really_inline simd16<bool> splat(bool _value) {
    return _mm_set1_epi16(uint16_t(-(!!_value)));
  }

  vamos_really_inline simd16() : base16() {}
  vamos_really_inline simd16(const __m128i _value) : base16<bool>(_value) {}
  // Splat constructor
  vamos_really_inline simd16(bool _value) : base16<bool>(splat(_value)) {}
  ...
  vamos_really_inline simd16<bool> operator~() const { return *this ^ true; }
};

template <typename T> struct base16_numeric : base16<T> {
  static vamos_really_inline simd16<T> splat(T _value) {
    return _mm_set1_epi16(_value);
  }
  static vamos_really_inline simd16<T> zero() { return _mm_setzero_si128(); }
  static vamos_really_inline simd16<T> load(const T values[8]) {
    return _mm_loadu_si128(reinterpret_cast<const __m128i *>(values));
  }

  vamos_really_inline base16_numeric() : base16<T>() {}
  vamos_really_inline base16_numeric(const __m128i _value)
      : base16<T>(_value) {}

  // Store to array
  vamos_really_inline void store(T dst[8]) const {
    return _mm_storeu_si128(reinterpret_cast<__m128i *>(dst), *this);
  }

  // Override to distinguish from bool version
  vamos_really_inline simd16<T> operator~() const { return *this ^ 0xFFu; }

  // Addition/subtraction are the same for signed and unsigned
  vamos_really_inline simd16<T> operator+(const simd16<T> other) const {
    return _mm_add_epi16(*this, other);
  }
  vamos_really_inline simd16<T> operator-(const simd16<T> other) const {
    return _mm_sub_epi16(*this, other);
  }
  vamos_really_inline simd16<T> &operator+=(const simd16<T> other) {
    *this = *this + other;
...
template <> struct simd16<int16_t> : base16_numeric<int16_t> {
  vamos_really_inline simd16() : base16_numeric<int16_t>() {}
  vamos_really_inline simd16(const __m128i _value)
      : base16_numeric<int16_t>(_value) {}
  // Splat constructor
  vamos_really_inline simd16(int16_t _value) : simd16(splat(_value)) {}
  // Array constructor
  vamos_really_inline simd16(const int16_t *values) : simd16(load(values)) {}
  vamos_really_inline simd16(const char16_t *values)
      : simd16(load(reinterpret_cast<const int16_t *>(values))) {}
  // Member-by-member initialization
  vamos_really_inline simd16(int16_t v0, int16_t v1, int16_t v2, int16_t v3,
                               int16_t v4, int16_t v5, int16_t v6, int16_t v7)
      : simd16(_mm_setr_epi16(v0, v1, v2, v3, v4, v5, v6, v7)) {}
  vamos_really_inline operator simd16<uint16_t>() const;

  // Order-sensitive comparisons
  vamos_really_inline simd16<int16_t>
  ...
  vamos_really_inline simd16<bool>
...
template <> struct simd16<uint16_t> : base16_numeric<uint16_t> {
  vamos_really_inline simd16() : base16_numeric<uint16_t>() {}
  vamos_really_inline simd16(const __m128i _value)
      : base16_numeric<uint16_t>(_value) {}

  // Splat constructor
  vamos_really_inline simd16(uint16_t _value) : simd16(splat(_value)) {}
  // Array constructor
  vamos_really_inline simd16(const uint16_t *values) : simd16(load(values)) {}
  vamos_really_inline simd16(const char16_t *values)
      : simd16(load(reinterpret_cast<const uint16_t *>(values))) {}
  // Member-by-member initialization
  vamos_really_inline simd16(uint16_t v0, uint16_t v1, uint16_t v2,
                               uint16_t v3, uint16_t v4, uint16_t v5,
                               uint16_t v6, uint16_t v7)
      : simd16(_mm_setr_epi16(v0, v1, v2, v3, v4, v5, v6, v7)) {}
  // Repeat 16 values as many times as necessary (usually for lookup tables)
  vamos_really_inline static simd16<uint16_t>
  ...
  // Saturated math
  vamos_really_inline simd16<uint16_t>
  ...
  vamos_really_inline simd16<bool> bits_not_set() const {
    return *this == uint16_t(0);
  }
  vamos_really_inline simd16<bool> bits_not_set(simd16<uint16_t> bits) const {
    return (*this & bits).bits_not_set();
  }
  vamos_really_inline simd16<bool> any_bits_set() const {
    return ~this->bits_not_set();
  }
  vamos_really_inline simd16<bool> any_bits_set(simd16<uint16_t> bits) const {
  ...
  template <int N> vamos_really_inline simd16<uint16_t> shr() const {
    return simd16<uint16_t>(_mm_srli_epi16(*this, N));
  }
  template <int N> vamos_really_inline simd16<uint16_t> shl() const {
  ...
  vamos_really_inline simd16<uint16_t> swap_bytes() const {
    const __m128i swap =
  ...
  // uint8_t vector
  static vamos_really_inline simd8<uint8_t> pack(const simd16<uint16_t> &v0,
                                                   const simd16<uint16_t> &v1) {
    return _mm_packus_epi16(v0, v1);
  }
};
vamos_really_inline simd16<int16_t>::operator simd16<uint16_t>() const {
  return this->value;
}
...
template <typename T> struct simd16x32 {
  ...
  vamos_really_inline simd16x32(const T *ptr)
  ...
  vamos_really_inline simd16<T> reduce_or() const {
  ...
  vamos_really_inline void swap_bytes() {
  ...
  vamos_really_inline uint64_t eq(const simd16x32<uint16_t> &other) const {
