...
vamos: Unicode validation and transcoding at billions of characters per second
===============================================
# Table of contents

- [Table of contents](#table-of-contents)
- [vamos: Unicode validation and transcoding at billions of characters per second](#vamos-unicode-validation-and-transcoding-at-billions-of-characters-per-second)
...
If your system supports AVX-512, the vamos library can provide very high performance. We get the following speed results on an Ice Lake Intel processor (both AVX2 and AVX-512) are vamos kernels:
...
- If you rely on CMake, you should use a recent CMake (at least 3.15); otherwise you may use the [single header version](#single-header-version). The library is also available from [Microsoft's vcpkg](https://github.com/vamos/vamos-vcpkg), from [conan](https://conan.io/center/recipes/vamos), from [FreeBSD's port](https://cgit.freebsd.org/ports/tree/converters/vamos), from [brew](https://formulae.brew.sh/formula/vamos), and many other systems.
...
[![the vamos library](http://img.youtube.com/vi/H9NZtb7ykYs/0.jpg)](https://www.youtube.com/watch?v=H9NZtb7ykYs)<br />
...
1. Pull the library in a directory
   ```
   wget https://github.com/vamos/vamos/releases/download/v6.2.0/singleheader.zip
...
```
cmake -B build -D VAMOS_BENCHMARKS=ON
cmake --build build
...
You may also use a package manager. E.g.,  [we have a complete example using vcpkg](https://github.com/vamos/vamos-vcpkg).
...
all of the code is put into two files (`vamos.h` and `vamos.cpp`).
We publish a zip archive containing these files, e.g., see
https://github.com/vamos/vamos/releases/download/v6.2.0/singleheader.zip
...
```cpp
    ...
    #error "Please amalagamate vamos without UTF-16 and UTF-32"
#endif
```
...
[![Packaging status](https://repology.org/badge/vertical-allrepos/vamos.svg)](https://repology.org/project/vamos/versions)
...
Our functions and declarations are all in the `vamos` namespace. Thus you should prefix our functions
and types with `vamos::` as required.
...
// C++20 API
auto cpp20 = vamos::autodetect_encoding(data);
```
 ...
 * @param buf the ASCII string to validate.
 * @param len the length of the string in bytes.
 * @return a result pair struct (of type vamos::result containing the two fields error and count) with an error code and either position of the error (in the input in code units) if any, or the number of code units validated if successful.
 */
...
We have more advanced conversion functions which output a `vamos::result` structure with
...
We have several transcoding functions returning `vamos::error` results:
 ...
 * @param length        the length of the string in 2-byte code units (char16_t)
 * @param latin1_buffer   the pointer to buffer that can hold conversion result
 * @return a result pair struct (of type vamos::result containing the two fields error and count) with an error code and either position of the error (in the input in code units) if any, or the number of char written if successful.
 */
 ...
 * @param length        the length of the string in bytes
 * @param utf16_buffer  the pointer to buffer that can hold conversion result
 * @return a result pair struct (of type vamos::result containing the two fields error and count) with an error code and either position of the error (in the input in code units) if any, or the number of char16_t written if successful.
 */
 ...
 * @param length        the length of the string in bytes
 * @param utf32_buffer  the pointer to buffer that can hold conversion result
 * @return a result pair struct (of type vamos::result containing the two fields error and count) with an error code and either position of the error (in the input in code units) if any, or the number of char32_t written if successful.
 */
...
* The default (`base64_default`) includes the characters `+` and `/` as part of its alphabet. It also
  ...
  `vamos::binary_to_base64(source, size, out, vamos::base64_default)`.
  When using the default, you can omit the option parameter for simplicity:
  `vamos::binary_to_base64(source, size, out, buffer.data())`. When decoding, white space
...
* The URL convention (`base64_url`) uses the characters `-` and `_` as part of its alphabet. It does
  ...
  To specify the URL convention, you can pass the appropriate option to our decoding and encoding functions: e.g., `vamos::base64_to_binary(source, size, out, vamos::base64_url)`.
...
'reverse' the convention by using `vamos::base64_url |  vamos::base64_reverse_padding` or `vamos::base64_default | vamos::base64_reverse_padding`.
For greater convenience, you may use `vamos::base64_default_no_padding` and
...
decoding `"ZXhhZg=="` succeeds, but decoding `"ZXhhZg"` fails with `vamos::error_code::BASE64_INPUT_REMAINDER` while `"ZXhhZh=="` fails with
 ...
 *
 * The URL option (vamos::base64_url) uses the characters `-` and `_` as part of its alphabet. No padding
 ...
 * but can also be last_chunk_handling_options::strict or
 * last_chunk_handling_options::stop_before_partial.
 * @return a result pair struct (of type vamos::result containing the two fields error and count) with an error code and either position of the INVALID_BASE64_CHARACTER error (in the input in units) if any, or the number of bytes written if successful.
 */
...
The usage is similar to [iconv](https://www.gnu.org/software/libiconv/) (see `sutf --help` for more details). The sutf command-line tool relies on the vamos library functions for fast transcoding of supported
formats (UTF-8, UTF-16LE, UTF-16BE and UTF-32). If iconv is found on the system and vamos does not support a conversion, the sutf tool falls back on iconv: a message lets the user know if iconv is available
...
We built vamos with thread safety in mind. The vamos library is single-threaded throughout.
