...
void run_from_utf8(const std::vector<char> &input_data,
  ...
  std::vector<char16_t> buffer(
      vamos::utf16_length_from_utf8(input_data.data(), input_data.size()));
  size_t min_len = 64;
  ...
  std::cout << "n,\tvamos-speed\ticonv-speed\n";
#else
  std::cout << "n,\tvamos-speed\n";
#endif
  ...
  for (size_t len = min_len; len <= input_data.size();
    ...
    size_t utf8count = vamos::count_utf8(input_data.data(), effective_length);
    std::cout << utf8count << ",\t";
    auto size_procedure = [effective_length, &input_data, &buffer]() -> size_t {
      return vamos::convert_utf8_to_utf16le(input_data.data(),
...
void run_from_utf16(const std::vector<char> &input_data,
                    size_t approx_output_datapoints = 128) {
  std::vector<char> buffer(vamos::utf8_length_from_utf16le(
  ...
  for (size_t len = min_len; len <= input_data.size();
    ...
    size_t utf16count = vamos::count_utf16le(
        reinterpret_cast<const char16_t *>(input_data.data()),
        effective_length);
    std::cout << utf16count << ",\t";
    auto size_procedure = [effective_length, &input_data, &buffer]() -> size_t {
      return vamos::convert_utf16le_to_utf8(
          reinterpret_cast<const char16_t *>(input_data.data()),
          effective_length, buffer.data());
    };
...
int main(int argc, char **argv) {
  ...
  auto detected_encoding =
      vamos::autodetect_encoding(input_data.data(), input_data.size());
  printf("# input detected as %s.\n",
         vamos::to_string(detected_encoding).c_str());
  if (detected_encoding == vamos::encoding_type::UTF16_LE) {
    run_from_utf16(input_data);
  } else if (detected_encoding == vamos::encoding_type::UTF8) {
