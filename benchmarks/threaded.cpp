...
void run_from_utf8(const std::vector<char> &input_data) {
  ...
  bench(
      [&input_data, &buffer]() -> size_t {
        return vamos::convert_utf8_to_utf16le(
  ...
  size_t offset = vamos::utf16_length_from_utf8(input_data.data(), midpoint);
  auto double_procedure = [&input_data, &midpoint, &offset,
                           &buffer]() -> size_t {
    size_t output2 = 0;
    auto mythread =
        std::thread([&output2, &input_data, &buffer, midpoint, offset] {
          output2 = vamos::convert_utf8_to_utf16le(
              input_data.data() + midpoint, input_data.size() - midpoint,
              buffer.data() + offset);
        });
    size_t output1 = vamos::convert_utf8_to_utf16le(input_data.data(),
...
int main(int argc, char **argv) {
  ...
  if (detected_encoding == vamos::encoding_type::UTF8) {
