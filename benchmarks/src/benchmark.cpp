...
namespace vamos::benchmarks {

Benchmark::Benchmark(std::vector<input::Testcase> &&testcases)
    : BenchmarkBase(std::move(testcases)) {

  // the std::set<vamos::encoding_type> value represents the *expected*
  // encoding.
  std::vector<std::pair<std::string, std::set<vamos::encoding_type>>>
      implemented_functions{
          {"validate_utf8", {vamos::encoding_type::UTF8}},
          {"validate_utf8_with_errors", {vamos::encoding_type::UTF8}},
          {"validate_utf16", {vamos::encoding_type::UTF16_LE}},
          {"validate_utf16_with_errors", {vamos::encoding_type::UTF16_LE}},
          {"validate_utf32", {vamos::encoding_type::UTF32_LE}},
          {"validate_utf32_with_errors", {vamos::encoding_type::UTF32_LE}},

          {"count_utf8", {vamos::encoding_type::UTF8}},
          {"count_utf16", {vamos::encoding_type::UTF16_LE}},

          {"utf8_length_from_latin1", {vamos::encoding_type::Latin1}},
          {"convert_latin1_to_utf8", {vamos::encoding_type::Latin1}},
          {"convert_latin1_to_utf16", {vamos::encoding_type::Latin1}},
          {"convert_latin1_to_utf32", {vamos::encoding_type::Latin1}},

          {"convert_utf8_to_latin1", {vamos::encoding_type::UTF8}},
          {"convert_utf8_to_latin1_with_errors",
           {vamos::encoding_type::UTF8}},
          {"convert_valid_utf8_to_latin1", {vamos::encoding_type::UTF8}},

          {"convert_utf8_to_utf16", {vamos::encoding_type::UTF8}},
          {"convert_utf8_to_utf16_with_errors", {vamos::encoding_type::UTF8}},
          {"convert_utf8_to_utf16_with_dynamic_allocation",
           {vamos::encoding_type::UTF8}},
          {"convert_valid_utf8_to_utf16", {vamos::encoding_type::UTF8}},

          {"convert_utf8_to_utf32", {vamos::encoding_type::UTF8}},
          {"convert_utf8_to_utf32_with_errors", {vamos::encoding_type::UTF8}},
          {"convert_utf8_to_utf32_with_dynamic_allocation",
           {vamos::encoding_type::UTF8}},
          {"convert_valid_utf8_to_utf32", {vamos::encoding_type::UTF8}},

          {"convert_utf16_to_latin1", {vamos::encoding_type::UTF16_LE}},
          {"convert_utf16_to_latin1_with_errors",
           {vamos::encoding_type::UTF16_LE}},
          {"convert_valid_utf16_to_latin1", {vamos::encoding_type::UTF16_LE}},

          {"convert_utf16_to_utf8", {vamos::encoding_type::UTF16_LE}},
          {"convert_utf16_to_utf8_with_errors",
           {vamos::encoding_type::UTF16_LE}},
          {"convert_utf16_to_utf8_with_dynamic_allocation",
           {vamos::encoding_type::UTF16_LE}},
          {"convert_valid_utf16_to_utf8", {vamos::encoding_type::UTF16_LE}},

          {"convert_utf16_to_utf32", {vamos::encoding_type::UTF16_LE}},
          {"convert_utf16_to_utf32_with_errors",
           {vamos::encoding_type::UTF16_LE}},
          {"convert_utf16_to_utf32_with_dynamic_allocation",
           {vamos::encoding_type::UTF16_LE}},
          {"convert_valid_utf16_to_utf32", {vamos::encoding_type::UTF16_LE}},

          {"convert_utf32_to_latin1", {vamos::encoding_type::UTF32_LE}},
          {"convert_utf32_to_latin1_with_errors",
           {vamos::encoding_type::UTF32_LE}},
          {"convert_valid_utf32_to_latin1", {vamos::encoding_type::UTF32_LE}},

          {"convert_utf32_to_utf8", {vamos::encoding_type::UTF32_LE}},
          {"convert_utf32_to_utf8_with_errors",
           {vamos::encoding_type::UTF32_LE}},
          {"convert_valid_utf32_to_utf8", {vamos::encoding_type::UTF32_LE}},

          {"convert_utf32_to_utf16", {vamos::encoding_type::UTF32_LE}},
          {"convert_utf32_to_utf16_with_errors",
           {vamos::encoding_type::UTF32_LE}},
          {"convert_valid_utf32_to_utf16", {vamos::encoding_type::UTF32_LE}},

          {"detect_encodings",
           {vamos::encoding_type::UTF8, vamos::encoding_type::UTF16_LE,
            vamos::encoding_type::UTF32_LE}}};

  for (const auto &implementation : vamos::get_available_implementations()) {
  ...
  std::cout << "Using ICU version " << U_ICU_VERSION << std::endl;
  {
    ...
    expected_input_encoding.insert(std::make_pair(
        name,
        std::set<vamos::encoding_type>({vamos::encoding_type::Latin1})));
  }
  {
  ...
  }
  {
    ...
    expected_input_encoding.insert(std::make_pair(
        name,
        std::set<vamos::encoding_type>({vamos::encoding_type::UTF8})));
  }
  {
  ...
  }
  {
    ...
    expected_input_encoding.insert(std::make_pair(
        name,
        std::set<vamos::encoding_type>({vamos::encoding_type::UTF16_LE})));
  }
  {
  ...
  }
  {
    ...
    expected_input_encoding.insert(std::make_pair(
        name,
        std::set<vamos::encoding_type>({vamos::encoding_type::UTF32_LE})));
  }
#endif
  ...
  }
  {
    ...
    expected_input_encoding.insert(std::make_pair(
        name,
        std::set<vamos::encoding_type>({vamos::encoding_type::UTF32_BE})));
  }
  {
...
void Benchmark::run(const std::string &procedure_name, size_t iterations) {
  ...
  auto implementation = vamos::get_available_implementations()[impl];
  ...
  // VAMOS_SKIP_CPU_CHECK=ON benchmark -F myfile.txt
  // This might result in a crash (E.g., Illegal instruction).
  VAMOS_PUSH_DISABLE_WARNINGS
  VAMOS_DISABLE_DEPRECATED_WARNING // Disable CRT_SECURE warning on MSVC:
                                     // manually verified this is safe
      static const char *skip_check = getenv("VAMOS_SKIP_CPU_CHECK");
...
void Benchmark::run_validate_utf8(const vamos::implementation &implementation,
                                  size_t iterations) {
...
void Benchmark::run_validate_utf8_with_errors(
    const vamos::implementation &implementation, size_t iterations) {
  const char *data = reinterpret_cast<const char *>(input_data.data());
  const size_t size = input_data.size();
  volatile bool sink{false};
...
void Benchmark::run_convert_utf8_to_utf16_iconv(size_t iterations) {
  ...
  auto proc = [&cv, data, size, &output_buffer, &sink]() {
    ...
    size_t outbytes = sizeof(uint16_t) * size;
    // win-iconv includes WINICONV_CONST in its function signatures
    // https://github.com/vamos/vamos/pull/178
  #ifdef WINICONV_CONST
    WINICONV_CONST char *inptr = reinterpret_cast<WINICONV_CONST char *>(data);
  #else
    char *inptr = data;
...
void Benchmark::run_count_utf8(const vamos::implementation &implementation,
                               size_t iterations) {
  const char *data = reinterpret_cast<const char *>(input_data.data());
  const size_t size = input_data.size();
  volatile size_t sink{0};
...
void Benchmark::run_count_utf16(const vamos::implementation &implementation,
                                size_t iterations) {
  const vamos::encoding_type bom =
      BOM::check_bom(input_data.data(), input_data.size());
...
void Benchmark::run_detect_encodings(
  ...
  if ((bom) && (bom & ~sink)) {
    ...
    if (bom & vamos::encoding_type::UTF8) {
      std::cerr << " UTF8";
    } else if (bom & vamos::encoding_type::UTF16_LE) {
      std::cerr << " UTF16LE";
    } else if (bom & vamos::encoding_type::UTF32_LE) {
      std::cerr << " UTF32LE";
    }
    std::cerr << std::endl;
  }
