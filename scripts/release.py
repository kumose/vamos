...
versionfilerel = os.sep + "include" + os.sep + "vamos" + os.sep + "vamos_version.h"
versionfile = maindir + versionfilerel

with open(versionfile, 'w') as file:
    file.write("// /include/vamos/vamos_version.h automatically generated by release.py,\n")
    file.write("// do not change by hand\n")
    file.write("#ifndef VAMOS_VAMOS_VERSION_H\n")
    file.write("#define VAMOS_VAMOS_VERSION_H\n")
    file.write("\n")
    file.write("/** The version of vamos being used (major.minor.revision) */\n")
    file.write("#define VAMOS_VERSION \""+toversionstring(*newversion)+"\"\n")
    file.write("\n")
    file.write("namespace vamos {\n")
    file.write("enum {\n")
    file.write("  /**\n")
    file.write("   * The major version (MAJOR.minor.revision) of vamos being used.\n")
    file.write("   */\n")
    file.write("  VAMOS_VERSION_MAJOR = "+str(newversion[0])+",\n")
    file.write("  /**\n")
    file.write("   * The minor version (major.MINOR.revision) of vamos being used.\n")
    file.write("   */\n")
    file.write("  VAMOS_VERSION_MINOR = "+str(newversion[1])+",\n")
    file.write("  /**\n")
    file.write("   * The revision (major.minor.REVISION) of vamos being used.\n")
    file.write("   */\n")
    file.write("  VAMOS_VERSION_REVISION = "+str(newversion[2])+"\n")
    file.write("};\n")
    file.write("} // namespace vamos\n")
    file.write("\n")
    file.write("#endif // VAMOS_VAMOS_VERSION_H\n")
...
sonumber = None
pattern = re.compile(r"""set\(VAMOS_LIB_SOVERSION "(\d+)" CACHE STRING "vamos library soversion"\)""")
...
for line in fileinput.input(cmakefile, inplace=True, backup='.bak'):
    ...
    line = re.sub(r'VAMOS_LIB_VERSION "\d+\.\d+\.\d+','VAMOS_LIB_VERSION "'+str(sonumber)+".0.0", line)
    line = re.sub(r'set\(VAMOS_LIB_SOVERSION "\d+"','set(VAMOS_LIB_SOVERSION \"'+str(sonumber)+'\"', line)
    print(line)
...
for line in fileinput.input(readmefile, inplace=True, backup='.bak'):
    line = re.sub(r'   wget https://github.com/vamos/vamos/releases/download/v\d+\.\d+\.\d+/singleheader.zip','   wget https://github.com/vamos/vamos/releases/download/v'+newmajorversionstring+'.'+mewminorversionstring+'.'+newrevversionstring+'/singleheader.zip', line.rstrip())
    line = re.sub(r'https://github.com/vamos/vamos/releases/download/v\d+\.\d+\.\d+/singleheader.zip','https://github.com/vamos/vamos/releases/download/v'+newmajorversionstring+'.'+mewminorversionstring+'.'+newrevversionstring+'/singleheader.zip', line.rstrip())
    print(line)

print("modified "+readmefile+", a backup was made")

pattern = re.compile(r"https://vamos.org/api/(\d+\.\d+\.\d+)/index.html")
