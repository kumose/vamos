...
void CommandLine::run() {
  if (output_file.empty()) {
    ...
    VAMOS_DISABLE_DEPRECATED_WARNING
  ...
  if (from_encoding == "UTF-8") {
    if (to_encoding == "UTF-16LE" || to_encoding == "UTF-16") {
      auto proc = [this, &fpout](size_t size) {
        ...
        size_t len = vamos::convert_utf8_to_utf16le(
      ...
      run_vamos_procedure(proc);
    } else if (to_encoding == "UTF-16BE") {
      auto proc = [this, &fpout](size_t size) {
        if (!(input_files.empty())) {
          size = find_last_leading_byte(size);
        }
        size_t len = vamos::convert_utf8_to_utf16be(
            reinterpret_cast<const char *>(input_data.data()), size,
            reinterpret_cast<char16_t *>(output_buffer.data()));
        if (len == 0 && size != 0) {
          printf("Could not convert %s\n",
      ...
      auto proc = [this, &fpout](size_t size) {
        ...
        size_t len = vamos::convert_utf8_to_utf32(
            reinterpret_cast<const char *>(input_data.data()), size,
    ...
    if (to_encoding == "UTF-8") {
      auto proc = [this, &fpout](size_t size_bytes) {
        ...
        size_t len = vamos::convert_utf16le_to_utf8(
      ...
      auto proc = [this, &fpout](size_t size_bytes) {
        ...
        const size_t size = size_bytes / 2;
        size_t len = vamos::convert_utf16le_to_utf32(
            reinterpret_cast<const char16_t *>(input_data.data()), size,
            reinterpret_cast<char32_t *>(output_buffer.data()));
        if (len == 0 && size != 0) {
          printf("Could not convert %s\n",
    ...
    if (to_encoding == "UTF-8") {
      auto proc = [this, &fpout](size_t size_bytes) {
        ...
        const size_t size = size_bytes / 2;
        size_t len = vamos::convert_utf16be_to_utf8(
            reinterpret_cast<const char16_t *>(input_data.data()), size,
            output_buffer.data());
        if (len == 0 && size != 0) {
          printf("Could not convert %s\n",
      ...
      auto proc = [this, &fpout](size_t size_bytes) {
        ...
        const size_t size = size_bytes / 2;
        size_t len = vamos::convert_utf16be_to_utf32(
            reinterpret_cast<const char16_t *>(input_data.data()), size,
            reinterpret_cast<char32_t *>(output_buffer.data()));
        if (len == 0 && size != 0) {
          printf("Could not convert %s\n",
    ...
    if (to_encoding == "UTF-8") {
      auto proc = [this, &fpout](size_t size_bytes) {
        ...
        size_t len = vamos::convert_utf32_to_utf8(
      ...
      auto proc = [this, &fpout](size_t size_bytes) {
        const size_t size = size_bytes / 4;
        size_t len = vamos::convert_utf32_to_utf16le(
            reinterpret_cast<const char32_t *>(input_data.data()), size,
            reinterpret_cast<char16_t *>(output_buffer.data()));
        if (len == 0 && size != 0) {
          printf("Could not convert %s\n",
      ...
      auto proc = [this, &fpout](size_t size_bytes) {
        const size_t size = size_bytes / 4;
        size_t len = vamos::convert_utf32_to_utf16be(
            reinterpret_cast<const char32_t *>(input_data.data()), size,
            reinterpret_cast<char16_t *>(output_buffer.data()));
        if (len == 0 && size != 0) {
          printf("Could not convert %s\n",
...
void CommandLine::run_vamos_procedure(PROCEDURE proc) {
  size_t leftovers{0};
...
#else
  fprintf(stderr,
          "Conversion from %s to %s is not supported by the vamos library "
...
void CommandLine::show_help() {
  ...
  printf(
      ...
      "vamos library for supported formats and iconv for other formats.\n");
#else
  printf("Convert encoding of input files from one encoding to another using "
         "vamos library for supported formats.\n");
#endif
...
void CommandLine::show_formats() {
  printf("Formats supported by vamos library: UTF-8, UTF-16LE, UTF-16BE, "
         "UTF-32LE\n");
#if ICONV_AVAILABLE
