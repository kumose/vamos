...
public:
  vamos_really_inline buf_block_reader(const uint8_t *_buf, size_t _len);
  vamos_really_inline size_t block_index();
  vamos_really_inline bool has_full_block() const;
  vamos_really_inline const uint8_t *full_block() const;
  /**
   ...
   */
  vamos_really_inline size_t get_remainder(uint8_t *dst) const;
  vamos_really_inline void advance();
...
vamos_unused static char *format_input_text_64(const uint8_t *text) {
  static char *buf =
...
// Routines to print masks and text for debugging bitmask operations
vamos_unused static char *format_input_text(const simd8x64<uint8_t> &in) {
  static char *buf =
      reinterpret_cast<char *>(malloc(sizeof(simd8x64<uint8_t>) + 1));
...
vamos_unused static char *format_mask(uint64_t mask) {
...
template <size_t STEP_SIZE>
vamos_really_inline size_t buf_block_reader<STEP_SIZE>::block_index() {
  return idx;
}

template <size_t STEP_SIZE>
vamos_really_inline bool buf_block_reader<STEP_SIZE>::has_full_block() const {
  return idx < lenminusstep;
}

template <size_t STEP_SIZE>
vamos_really_inline const uint8_t *
buf_block_reader<STEP_SIZE>::full_block() const {
  return &buf[idx];
}

template <size_t STEP_SIZE>
vamos_really_inline size_t
...
template <size_t STEP_SIZE>
vamos_really_inline void buf_block_reader<STEP_SIZE>::advance() {
  idx += STEP_SIZE;
}
