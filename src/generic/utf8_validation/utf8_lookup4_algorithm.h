...
//
vamos_really_inline simd8<uint8_t> is_incomplete(const simd8<uint8_t> input) {
...
struct utf8_checker {
  ...
  //
  vamos_really_inline void check_utf8_bytes(const simd8<uint8_t> input,
                                              const simd8<uint8_t> prev_input) {
  ...
  vamos_really_inline void check_next_input(const simd8x64<uint8_t> &input) {
    if (vamos_likely(is_ascii(input))) {
      this->error |= this->prev_incomplete;
    } else {
