...
vamos_really_inline static result
...
implementation::convert_utf16le_to_latin1_with_errors(
    const char16_t *src, size_t len, char *dst) const noexcept {
  return rvv_utf16_to_latin1_with_errors<vamos_ByteFlip::NONE>(src, len, dst);
}
...
implementation::convert_utf16be_to_latin1_with_errors(
    const char16_t *src, size_t len, char *dst) const noexcept {
  if (supports_zvbb())
    return rvv_utf16_to_latin1_with_errors<vamos_ByteFlip::ZVBB>(src, len,
                                                                   dst);
  else
    return rvv_utf16_to_latin1_with_errors<vamos_ByteFlip::V>(src, len, dst);
}
...
rvv_utf16_to_utf8_with_errors(const char16_t *src, size_t len, char *dst) {
  ...
  for (size_t vl, vlOut; n > 0;) {
    ...
    if (tail)
      while (n) {
        uint16_t word = vamos_byteflip<bflip>(src[0]);
        if ((word & 0xFF80) == 0) {
          break;
          ...
          if (diff > 0x3FF)
            return result(error_code::SURROGATE, src - srcBeg);
          uint16_t diff2 = vamos_byteflip<bflip>(src[1]) - 0xDC00;
          if (diff2 > 0x3FF)
            return result(error_code::SURROGATE, src - srcBeg);
...
vamos_warn_unused result implementation::convert_utf16le_to_utf8_with_errors(
    const char16_t *src, size_t len, char *dst) const noexcept {
  return rvv_utf16_to_utf8_with_errors<vamos_ByteFlip::NONE>(src, len, dst);
}

vamos_warn_unused result implementation::convert_utf16be_to_utf8_with_errors(
    const char16_t *src, size_t len, char *dst) const noexcept {
  if (supports_zvbb())
    return rvv_utf16_to_utf8_with_errors<vamos_ByteFlip::ZVBB>(src, len, dst);
  else
    return rvv_utf16_to_utf8_with_errors<vamos_ByteFlip::V>(src, len, dst);
}
...
rvv_utf16_to_utf32_with_errors(const char16_t *src, size_t len, char32_t *dst) {
  ...
  while (len > 0) {
    ...
    if ((vamos_byteflip<bflip>(src[0]) & LO_SURROGATE_MASK) ==
        LO_SURROGATE_VALUE) {
      return result(error_code::SURROGATE, src - srcBeg);
    }
...

    {
      ...
      if (idx >= 0) {
        uint16_t word = vamos_byteflip<bflip>(src[idx]);
    ...
    last = vamos_byteflip<bflip>(src[vl]);
...
vamos_warn_unused result implementation::convert_utf16le_to_utf32_with_errors(
    const char16_t *src, size_t len, char32_t *dst) const noexcept {
  return rvv_utf16_to_utf32_with_errors<vamos_ByteFlip::NONE>(src, len, dst);
}

vamos_warn_unused result implementation::convert_utf16be_to_utf32_with_errors(
    const char16_t *src, size_t len, char32_t *dst) const noexcept {
  if (supports_zvbb())
    return rvv_utf16_to_utf32_with_errors<vamos_ByteFlip::ZVBB>(src, len,
                                                                  dst);
  else
    return rvv_utf16_to_utf32_with_errors<vamos_ByteFlip::V>(src, len, dst);
}
