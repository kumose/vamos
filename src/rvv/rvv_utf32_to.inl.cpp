...
rvv_convert_utf32_to_utf16_with_errors(const char32_t *src, size_t len,
  ...
  for (size_t vl, vlOut; len > 0; len -= vl, src += vl, dst += vlOut) {
    ...
    if (idx < 0) {
      vlOut = vl;
      vuint16m2_t n =
          vamos_byteflip<bflip>(__riscv_vncvt_x_x_w_u16m2(v, vlOut), vlOut);
...
vamos_warn_unused result implementation::convert_utf32_to_utf16le_with_errors(
    const char32_t *src, size_t len, char16_t *dst) const noexcept {
  return rvv_convert_utf32_to_utf16_with_errors<vamos_ByteFlip::NONE>(
      src, len, dst);
}
...
vamos_warn_unused result implementation::convert_utf32_to_utf16be_with_errors(
    const char32_t *src, size_t len, char16_t *dst) const noexcept {
  if (supports_zvbb())
    return rvv_convert_utf32_to_utf16_with_errors<vamos_ByteFlip::ZVBB>(
        src, len, dst);
  else
    return rvv_convert_utf32_to_utf16_with_errors<vamos_ByteFlip::V>(src, len,
                                                                       dst);
}
...
vamos_warn_unused size_t implementation::convert_valid_utf32_to_utf16le(
    const char32_t *src, size_t len, char16_t *dst) const noexcept {
  return rvv_convert_valid_utf32_to_utf16<vamos_ByteFlip::NONE>(src, len,
                                                                  dst);
}
...
vamos_warn_unused size_t implementation::convert_valid_utf32_to_utf16be(
    const char32_t *src, size_t len, char16_t *dst) const noexcept {
  if (supports_zvbb())
    return rvv_convert_valid_utf32_to_utf16<vamos_ByteFlip::ZVBB>(src, len,
                                                                    dst);
  else
    return rvv_convert_valid_utf32_to_utf16<vamos_ByteFlip::V>(src, len, dst);
}
