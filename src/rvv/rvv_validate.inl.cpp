...
vamos_really_inline static size_t rvv_count_valid_utf8(const char *src,
                                                         size_t len) {
  const char *beg = src;
  if (len < 32)
    return 0;
  ...
  for (size_t vl; n > 0; n -= vl, src += vl) {
    ...
    vuint8m4_t err1 = vamos_vrgather_u8m1x4(err1tbl, idx1);
    vuint8m4_t err2 = vamos_vrgather_u8m1x4(err2tbl, idx2);
    vuint8m4_t err3 = vamos_vrgather_u8m1x4(err3tbl, idx3);
...
rvv_validate_utf16_with_errors(const char16_t *src, size_t len) {
  ...
  for (size_t vl; len > 0;
    ...
    if (idx >= 0) {
      last = idx > 0 ? vamos_byteflip<bflip>(src[idx - 1]) : last;
...
implementation::validate_utf16le(const char16_t *src,
  ...
  return rvv_validate_utf16_with_errors<vamos_ByteFlip::NONE>(src, len)
...
vamos_warn_unused result implementation::validate_utf16be_with_errors(
    const char16_t *src, size_t len) const noexcept {
  if (supports_zvbb())
    return rvv_validate_utf16_with_errors<vamos_ByteFlip::ZVBB>(src, len);
  else
    return rvv_validate_utf16_with_errors<vamos_ByteFlip::V>(src, len);
}
