...
template <typename Tdst, vamos_ByteFlip bflip, bool validate = true>
vamos_really_inline static size_t rvv_utf8_to_common(char const *src,
                                                       size_t len, Tdst *dst) {
  ...
  constexpr endianness endian =
      bflip == vamos_ByteFlip::NONE ? endianness::LITTLE : endianness::BIG;
  ...
  for (size_t vl, vlOut; n > 0; n -= vl, src += vl, dst += vlOut) {
    ...
    if ((max | next0 | next1 | next2) < 0b10000000) {
      ...
      if (is16)
        __riscv_vse16_v_u16m4(
            (uint16_t *)dst,
            vamos_byteflip<bflip>(__riscv_vzext_vf2_u16m4(v0, vlOut), vlOut),
            vlOut);
      else
    ...
    if (validate) {
      ...
      vuint8m2_t err1 = vamos_vrgather_u8m1x2(err1tbl, idx1);
      vuint8m2_t err2 = vamos_vrgather_u8m1x2(err2tbl, idx2);
      vuint8m2_t err3 = vamos_vrgather_u8m1x2(err3tbl, idx3);
    ...
    if (max < 0b11100000) {
      ...
      if (is16)
        __riscv_vse16_v_u16m4((uint16_t *)dst,
                              vamos_byteflip<bflip>(b12, vlOut), vlOut);
      else
        __riscv_vse32_v_u32m8((uint32_t *)dst,
    ...
    if (max < 0b11110000) {
      ...
      if (is16)
        __riscv_vse16_v_u16m4((uint16_t *)dst,
                              vamos_byteflip<bflip>(b123, vlOut), vlOut);
      else
        __riscv_vse32_v_u32m8((uint32_t *)dst,
     ...
     */
  #define VAMOS_RVV_UTF8_TO_COMMON_M1(idx)                                   \
    ...
    {
      size_t vlDst;
      VAMOS_RVV_UTF8_TO_COMMON_M1(1)
      vlOut = vlDst;
    }

  #undef VAMOS_RVV_UTF8_TO_COMMON_M1
  }
  ...
  if (len > tail) {
    ...
    if (is16) {
      ...
      if (vamos_byteflip<bflip>((uint16_t)dst[-1]) >= 0xD800 &&
          vamos_byteflip<bflip>((uint16_t)dst[-1]) <= 0xDBFF)
        --dst;
    }
  }
...
vamos_warn_unused size_t implementation::convert_utf8_to_utf16le(
  ...
  return rvv_utf8_to_common<uint16_t, vamos_ByteFlip::NONE>(src, len,
                                                              (uint16_t *)dst);
}
...
vamos_warn_unused size_t implementation::convert_utf8_to_utf16be(
    const char *src, size_t len, char16_t *dst) const noexcept {
  if (supports_zvbb())
    return rvv_utf8_to_common<uint16_t, vamos_ByteFlip::ZVBB>(
        src, len, (uint16_t *)dst);
  else
    return rvv_utf8_to_common<uint16_t, vamos_ByteFlip::V>(src, len,
                                                             (uint16_t *)dst);
}
...
vamos_warn_unused size_t implementation::convert_valid_utf8_to_utf16le(
    const char *src, size_t len, char16_t *dst) const noexcept {
  return rvv_utf8_to_common<uint16_t, vamos_ByteFlip::NONE, false>(
      src, len, (uint16_t *)dst);
}
...
vamos_warn_unused size_t implementation::convert_valid_utf8_to_utf16be(
    const char *src, size_t len, char16_t *dst) const noexcept {
  if (supports_zvbb())
    return rvv_utf8_to_common<uint16_t, vamos_ByteFlip::ZVBB, false>(
        src, len, (uint16_t *)dst);
  else
    return rvv_utf8_to_common<uint16_t, vamos_ByteFlip::V, false>(
        src, len, (uint16_t *)dst);
}

vamos_warn_unused size_t implementation::convert_utf8_to_utf32(
    const char *src, size_t len, char32_t *dst) const noexcept {
  return rvv_utf8_to_common<uint32_t, vamos_ByteFlip::NONE>(src, len,
                                                              (uint32_t *)dst);
}
...
vamos_warn_unused size_t implementation::convert_valid_utf8_to_utf32(
    const char *src, size_t len, char32_t *dst) const noexcept {
  return rvv_utf8_to_common<uint32_t, vamos_ByteFlip::NONE, false>(
