...
lasx_convert_utf16_to_utf8(const char16_t *buf, size_t len, char *utf8_out) {
  ...
  while (end - buf >= std::ptrdiff_t(16 + safety_margin)) {
    ...
    if (__lasx_xbz_v(__lasx_xvslt_hu(v_07ff, in))) {
      ...
      // 4. pack the bytes
      const uint8_t *row1 = &vamos::tables::utf16_to_utf8::pack_1_2_utf8_bytes
      ...
      const uint8_t *row2 = &vamos::tables::utf16_to_utf8::pack_1_2_utf8_bytes
    ...
    if (__lasx_xbz_v(surrogates_bytemask)) {
      ...
      const uint8_t *row0 =
          &vamos::tables::utf16_to_utf8::pack_1_2_3_utf8_bytes[mask & 0xFF]
                                                                [0];
