...
arm_convert_utf16_to_utf8(const char16_t *buf, size_t len, char *utf8_out) {
  ...
  while (end - buf >= std::ptrdiff_t(16 + safety_margin)) {
    ...
    if (vmaxvq_u16(in) <= 0x7FF) {
      ...
      const uint16x8_t mask = vamos_make_uint16x8_t(
      ...
      // 4. pack the bytes
      const uint8_t *row =
          &vamos::tables::utf16_to_utf8::pack_1_2_utf8_bytes[m2][0];
    ...
    if (vmaxvq_u16(surrogates_bytemask) == 0) {
...
#ifdef VAMOS_REGULAR_VISUAL_STUDIO
      const uint16x8_t dup_even = vamos_make_uint16x8_t(
      ...
      */
       ...
       */
#define vamos_vec(x) vmovq_n_u16(static_cast<uint16_t>(x))
      // [aaaa|bbbb|bbcc|cccc] => [bbcc|cccc|bbcc|cccc]
      const uint16x8_t t0 = vreinterpretq_u16_u8(
          vqtbl1q_u8(vreinterpretq_u8_u16(in), vreinterpretq_u8_u16(dup_even)));
      // [bbcc|cccc|bbcc|cccc] => [00cc|cccc|0bcc|cccc]
      const uint16x8_t t1 = vandq_u16(t0, vamos_vec(0b0011111101111111));
      // [00cc|cccc|0bcc|cccc] => [10cc|cccc|0bcc|cccc]
      const uint16x8_t t2 = vorrq_u16(t1, vamos_vec(0b1000000000000000));
      ...
      const uint16x8_t s1 = vandq_u16(in, vamos_vec(0b0000111111000000));
      ...
      const uint16x8_t s3 = vorrq_u16(s2, vamos_vec(0b1100000011100000));
      const uint16x8_t v_07ff = vmovq_n_u16((uint16_t)0x07FF);
      const uint16x8_t one_or_two_bytes_bytemask = vcleq_u16(in, v_07ff);
      const uint16x8_t m0 =
          vbicq_u16(vamos_vec(0b0100000000000000), one_or_two_bytes_bytemask);
...
#undef vamos_vec
      ...
      const uint16x8_t one_byte_bytemask = vcleq_u16(in, v_007f);
#ifdef VAMOS_REGULAR_VISUAL_STUDIO
      const uint16x8_t onemask = vamos_make_uint16x8_t(
          0x0001, 0x0004, 0x0010, 0x0040, 0x0100, 0x0400, 0x1000, 0x4000);
      const uint16x8_t twomask = vamos_make_uint16x8_t(
