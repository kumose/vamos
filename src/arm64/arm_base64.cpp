...
static inline void compress(uint8x16_t data, uint16_t mask, char *output) {
  ...
  const uint8x16_t off =
      vamos_make_uint8x16_t(0, 0, 0, 0, 0, 0, 0, 0, 8, 8, 8, 8, 8, 8, 8, 8);
#else
...
template <bool base64_url> uint64_t to_base64_mask(block64 *b, bool *error) {
  ...
  if (base64_url) {
    lut_lo =
        vamos_make_uint8x16_t(0x3a, 0x70, 0x70, 0x70, 0x70, 0x70, 0x70, 0x70,
  ...
  if (base64_url) {
    lut_hi =
        vamos_make_uint8x16_t(0x11, 0x20, 0x42, 0x80, 0x8, 0x4, 0x8, 0x4,
  ...
  *error = checks > 0x3;
  ...
  if (base64_url) {
    roll_lut =
        vamos_make_uint8x16_t(0xe0, 0x11, 0x13, 0x4, 0xbf, 0xbf, 0xb9, 0xb9,
                                0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0);
  } else {
    roll_lut =
        vamos_make_uint8x16_t(0x0, 0x10, 0x13, 0x4, 0xbf, 0xbf, 0xb9, 0xb9,
                                0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0);
  }
#else
  if (base64_url) {
