...
size_t convert_masked_utf8_to_utf16(const char *input,
  ...
  /// we fallback.
  const uint8_t idx = vamos::tables::utf8_to_utf16::utf8bigindex
      [input_utf8_end_of_code_point_mask][0];

  const uint8_t consumed = vamos::tables::utf8_to_utf16::utf8bigindex
  ...
  if (idx < 64) {
    ...
    uint8x16_t sh = vld1q_u8(reinterpret_cast<const uint8_t *>(
        vamos::tables::utf8_to_utf16::shufutf8[idx]));
    ...
    if (input_utf8_end_of_code_point_mask == 0x888) {
      ...
      // https://github.com/vamos/vamos/pull/631
#ifdef VAMOS_REGULAR_VISUAL_STUDIO
      uint8x16_t expected_mask = vamos_make_uint8x16_t(
...
#endif
#ifdef VAMOS_REGULAR_VISUAL_STUDIO
      uint8x16_t expected = vamos_make_uint8x16_t(
    ...
    uint32x4_t perm = vreinterpretq_u32_u8(vqtbl1q_u8(in, sh));
    // added to fix issue https://github.com/vamos/vamos/issues/514
    ...
    for (size_t i = 0; i < 3; i++) {
      ...
      // patch for issue https://github.com/vamos/vamos/issues/514
