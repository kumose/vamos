...
vamos_warn_unused result implementation::convert_utf8_to_utf32_with_errors(
  ...
  if (std::get<0>(ret) == end) {
    return {vamos::SUCCESS, saved_bytes};
  }
  ...
  if (std::get<0>(ret) != end) {
    ...
    if (scalar_result.error != vamos::SUCCESS) {
  ...
  return {vamos::SUCCESS, size_t(std::get<1>(ret) - utf32_output)};
}
...
vamos_warn_unused result implementation::convert_utf16le_to_utf32_with_errors(
  ...
  return vamos::result(vamos::SUCCESS, saved_bytes);
}
