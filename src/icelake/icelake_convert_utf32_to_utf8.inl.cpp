...
avx512_convert_utf32_to_utf8(const char32_t *buf, size_t len,
  ...
  const size_t safety_margin =
      12; // to avoid overruns, see issue
          // https://github.com/vamos/vamos/issues/92
  ...
  while (end - buf >= std::ptrdiff_t(16 + safety_margin)) {
    ...
    if (one_or_two_bytes_bitmask == 0xffffffff) {
      ...
      const uint8_t *row =
          &vamos::tables::utf16_to_utf8::pack_1_2_utf8_bytes[uint8_t(M2)][0];
      const uint8_t *row_2 =
          &vamos::tables::utf16_to_utf8::pack_1_2_utf8_bytes[uint8_t(M2 >>
                                                                       16)][0];
    ...
    if (saturation_bitmask == 0xffffffff) {
      ...
      */
       ...
       */
#define vamos_vec(x) _mm256_set1_epi16(static_cast<uint16_t>(x))
      // [aaaa|bbbb|bbcc|cccc] => [bbcc|cccc|bbcc|cccc]
      const __m256i t0 = _mm256_shuffle_epi8(in_16, dup_even);
      // [bbcc|cccc|bbcc|cccc] => [00cc|cccc|0bcc|cccc]
      const __m256i t1 = _mm256_and_si256(t0, vamos_vec(0b0011111101111111));
      // [00cc|cccc|0bcc|cccc] => [10cc|cccc|0bcc|cccc]
      const __m256i t2 = _mm256_or_si256(t1, vamos_vec(0b1000000000000000));

      // [aaaa|bbbb|bbcc|cccc] =>  [0000|aaaa|bbbb|bbcc]
      const __m256i s0 = _mm256_srli_epi16(in_16, 4);
      // [0000|aaaa|bbbb|bbcc] => [0000|aaaa|bbbb|bb00]
      const __m256i s1 = _mm256_and_si256(s0, vamos_vec(0b0000111111111100));
      // [0000|aaaa|bbbb|bb00] => [00bb|bbbb|0000|aaaa]
      const __m256i s2 = _mm256_maddubs_epi16(s1, vamos_vec(0x0140));
      // [00bb|bbbb|0000|aaaa] => [11bb|bbbb|1110|aaaa]
      const __m256i s3 = _mm256_or_si256(s2, vamos_vec(0b1100000011100000));
      const __m256i m0 = _mm256_andnot_si256(one_or_two_bytes_bytemask,
                                             vamos_vec(0b0100000000000000));
...
#undef vamos_vec
      ...
      const uint8_t *row0 =
          &vamos::tables::utf16_to_utf8::pack_1_2_3_utf8_bytes[mask0][0];
      ...
      const uint8_t *row1 =
          &vamos::tables::utf16_to_utf8::pack_1_2_3_utf8_bytes[mask1][0];
      ...
      const uint8_t *row2 =
          &vamos::tables::utf16_to_utf8::pack_1_2_3_utf8_bytes[mask2][0];
      ...
      const uint8_t *row3 =
          &vamos::tables::utf16_to_utf8::pack_1_2_3_utf8_bytes[mask3][0];
