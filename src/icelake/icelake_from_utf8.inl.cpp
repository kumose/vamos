...
validating_utf8_to_fixed_length(const char *str, size_t len, OUTPUT *dwords) {
   ...
   * https://github.com/vamos/vamos/issues/471
   */
  while (end - ptr >= 64 + 4) {
    const __m512i utf8 = _mm512_loadu_si512((const __m512i *)ptr);
    if (checker.check_next_input(utf8)) {
      VAMOS_ICELAKE_STORE_ASCII(UTF32, utf8, output)
      output += 64;
      ptr += 64;
      continue;
    }
    ...
    if (valid_count0 + valid_count1 <= 16) {
      ...
      VAMOS_ICELAKE_WRITE_UTF16_OR_UTF32(vec0, valid_count0, true)
    } else {
      vec0 = expand_utf8_to_utf32(vec0);
      vec1 = expand_utf8_to_utf32(vec1);
      VAMOS_ICELAKE_WRITE_UTF16_OR_UTF32(vec0, valid_count0, true)
      VAMOS_ICELAKE_WRITE_UTF16_OR_UTF32(vec1, valid_count1, true)
    }
    ...
    if (valid_count2 + valid_count3 <= 16) {
      ...
      VAMOS_ICELAKE_WRITE_UTF16_OR_UTF32(vec2, valid_count2, true)
    } else {
      vec2 = expand_utf8_to_utf32(vec2);
      vec3 = expand_utf8_to_utf32(vec3);
      VAMOS_ICELAKE_WRITE_UTF16_OR_UTF32(vec2, valid_count2, true)
      VAMOS_ICELAKE_WRITE_UTF16_OR_UTF32(vec3, valid_count3, true)
    }
    ptr += 4 * 16;
  }
  ...
  if (end - ptr >= 64) {
    ...
    if (checker.check_next_input(utf8)) {
      ...
      const __m512i lane3 = broadcast_epi128<3>(utf8);
      VAMOS_ICELAKE_TRANSCODE16(lane2, lane3, true)
