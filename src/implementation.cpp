...
static_assert(sizeof(uint8_t) == sizeof(char),
              "vamos requires that uint8_t be a char");
static_assert(sizeof(uint16_t) == sizeof(char16_t),
              "vamos requires that char16_t be 16 bits");
static_assert(sizeof(uint32_t) == sizeof(char32_t),
              "vamos requires that char32_t be 32 bits");
// next line is redundant, but it is kept to catch defective systems.
static_assert(CHAR_BIT == 8, "vamos requires 8-bit bytes");
...
vamos_warn_unused encoding_type implementation::autodetect_encoding(
...
vamos_warn_unused size_t implementation::maximal_binary_length_from_base64(
    const char *input, size_t length) const noexcept {
...
vamos_warn_unused size_t implementation::base64_length_from_binary(
...
#define VAMOS_SINGLE_IMPLEMENTATION                                          \
  (VAMOS_IMPLEMENTATION_ICELAKE + VAMOS_IMPLEMENTATION_HASWELL +           \
       VAMOS_IMPLEMENTATION_WESTMERE + VAMOS_IMPLEMENTATION_ARM64 +        \
       VAMOS_IMPLEMENTATION_PPC64 + VAMOS_IMPLEMENTATION_LSX +             \
       VAMOS_IMPLEMENTATION_LASX + VAMOS_IMPLEMENTATION_FALLBACK ==        \
   1)
...
#if VAMOS_SINGLE_IMPLEMENTATION
...
public:
  ...
  vamos_warn_unused result validate_utf8_with_errors(
  ...
  vamos_warn_unused result validate_ascii_with_errors(
      const char *buf, size_t len) const noexcept final override {
...
public:
  vamos_warn_unused int detect_encodings(const char *,
                                           size_t) const noexcept override {
    return encoding_type::unspecified;
  }


  vamos_warn_unused bool validate_utf8(const char *,
  ...
  vamos_warn_unused size_t count_utf8(const char *,
                                        size_t) const noexcept final override {
    return 0;
  }
...
detect_best_supported_implementation_on_first_use::set_best() const noexcept {
  ...
  VAMOS_DISABLE_DEPRECATED_WARNING // Disable CRT_SECURE warning on MSVC:
      ...
      char *force_implementation_name = getenv("VAMOS_FORCE_IMPLEMENTATION");
 ...
 */
VAMOS_DLLIMPORTEXPORT const internal::available_implementation_list &
 ...
 */
VAMOS_DLLIMPORTEXPORT internal::atomic_ptr<const implementation> &
...
#endif
#define VAMOS_GET_CURRENT_IMPLEMENTION
...
vamos_warn_unused size_t convert_latin1_to_utf8(const char *buf, size_t len,
...
vamos_warn_unused size_t convert_utf16_to_utf8(const char16_t *buf,
                                                 size_t len,
...
vamos_warn_unused size_t convert_utf16le_to_utf8(const char16_t *buf,
                                                   size_t len,
                                                   char *utf8_buffer) noexcept {
  return get_default_implementation()->convert_utf16le_to_utf8(buf, len,
                                                               utf8_buffer);
}
vamos_warn_unused size_t convert_utf16be_to_utf8(const char16_t *buf,
                                                   size_t len,
                                                   char *utf8_buffer) noexcept {
...
vamos_warn_unused size_t convert_utf32_to_utf8(const char32_t *buf,
                                                 size_t len,
                                                 char *utf8_buffer) noexcept {
...
vamos_warn_unused size_t latin1_length_from_utf8(const char *buf,
                                                   size_t len) noexcept {
...
vamos_warn_unused size_t utf8_length_from_latin1(const char *buf,
                                                   size_t len) noexcept {
...
vamos_warn_unused result base64_to_binary_safe_impl(
...
vamos_warn_unused size_t convert_latin1_to_utf8_safe(
  ...
  while (true) {
    ...
    const auto write_len =
        vamos::convert_latin1_to_utf8(buf, read_len, utf8_output);
...
vamos_warn_unused result base64_to_binary_safe(
...
vamos_warn_unused int detect_encodings(const char *buf,
                                         size_t length) noexcept {
...
const implementation *builtin_implementation() {
  static const implementation *builtin_impl =
      get_available_implementations()[VAMOS_STRINGIFY(
